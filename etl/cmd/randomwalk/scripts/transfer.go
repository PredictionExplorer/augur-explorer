...
func main() {
	...
	if len(os.Args) < 5 {
		fmt.Printf(
			"Usage: \n\t\t%v [private_key] [rwalk_addr] [token_id] [new_owner_addr]\n\t\t"+
	...
	dst_addr := common.HexToAddress(os.Args[4])
	...
	fmt.Printf("Sending token %v to address = %v\n",token_id,dst_addr.String())
	tx,err := rwalk_ctrct.TransferFrom(txopts,from_address,dst_addr,big.NewInt(token_id))
	if err!=nil {
