// Sets recipient to receive charity deposits
package main
...
func main() {
	...
	if len(os.Args) != 4 {
		fmt.Printf("Usage: \n\t\t%v [priv_key] [contract_addr] [charity_percentage]\n\n\t\tSets new charity percentage\n",os.Args[0])
		os.Exit(1)
	}
	...
	cosmicgame_addr := common.HexToAddress(os.Args[2])
	num_str := os.Args[3]
	percentage_int,err := strconv.ParseInt(num_str,10,64)
	if err != nil {
		fmt.Printf("error parsing percentage parameter: %v\n",err)
		os.Exit(1)
	}
	...
	tx,err := cosmicgame_ctrct.SetCharityPercentage(txopts,big.NewInt(percentage_int))
