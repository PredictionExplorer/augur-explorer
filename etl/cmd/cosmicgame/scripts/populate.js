...
async function main() {
  ...
  await cosmicGameProxy.donateWithInfo(donationData,{value: hre.ethers.parseEther('6')});
  ...
  await cosmicGameProxy.connect(addr2).donateWithInfo(donationData,{value: hre.ethers.parseEther('6')});
  ...
  const contractBalance = await ethers.provider.getBalance(await cosmicGameProxy.getAddress());
  ...
  await cosmicGameProxy.connect(addr5).claimPrize({gasLimit:30000000});
  ...
  await cosmicGameProxy.connect(addr1).claimPrize({gasLimit:3000000});
  ...
  prizeTime = await cosmicGameProxy.timeUntilPrize();
  await ethers.provider.send("evm_increaseTime", [Number(prizeTime)]);
  tx = await cosmicGameProxy.connect(addr1).claimPrize({gasLimit:5000000});
  ...
  await randomWalkNFT.connect(addr2).setApprovalForAll(await cosmicGameProxy.getAddress(), true);
  await randomWalkNFT.connect(addr3).setApprovalForAll(await cosmicGameProxy.getAddress(), true);
	
  bidPrice = await cosmicGameProxy.getBidPrice();
  token_id = await mint_rwalk(addr1);
  bidParams = {msg:'donated token_id='+token_id,rwalk:-1};
  params = ethers.AbiCoder.defaultAbiCoder().encode([bidParamsEncoding],[bidParams])
  await cosmicGameProxy.connect(addr1).bidAndDonateNFT(params, await randomWalkNFT.getAddress(), token_id, { value: bidPrice });

  bidPrice = await cosmicGameProxy.getBidPrice();
  token_id = await mint_rwalk(addr2);
  bidParams = {msg:'me donated token_id='+token_id,rwalk:-1};
  params = ethers.AbiCoder.defaultAbiCoder().encode([bidParamsEncoding],[bidParams])
  await cosmicGameProxy.connect(addr2).bidAndDonateNFT(params, await randomWalkNFT.getAddress(), token_id, { value: bidPrice });

  bidPrice = await cosmicGameProxy.getBidPrice();
  token_id = await mint_rwalk(addr3);
  bidParams = {msg:'me donated token_id='+token_id,rwalk:-1};
  params = ethers.AbiCoder.defaultAbiCoder().encode([bidParamsEncoding],[bidParams])
  await cosmicGameProxy.connect(addr3).bidAndDonateNFT(params, await randomWalkNFT.getAddress(), token_id, { value: bidPrice });
  ...
  await ethers.provider.send("evm_increaseTime", [Number(prizeTime)]);
  await ethers.provider.send("evm_mine");
  tx = await cosmicGameProxy.connect(addr3).claimPrize({gasLimit:3000000});
  receipt = await tx.wait();

  await cosmicGameProxy.connect(owner).setCharity(await charityWallet.getAddress());
  await cosmicGameProxy.connect(owner).setRandomWalk(await randomWalkNFT.getAddress());
  await cosmicGameProxy.connect(owner).setRaffleWallet(await raffleWallet.getAddress());
  await cosmicGameProxy.connect(owner).setStakingWalletCST(await stakingWalletCST.getAddress());
  await cosmicGameProxy.connect(owner).setStakingWalletRWalk(await stakingWalletRWalk.getAddress());
  ...
  await cosmicGameProxy.connect(owner).setTokenContract(await cosmicToken.getAddress());
  await cosmicGameProxy.connect(owner).setNftContract(await cosmicSignature.getAddress());
  ...
  await cosmicGameProxy.connect(owner).setStartingBidPriceCSTMinLimit(150000000000000000000n);
  await cosmicGameProxy.connect(owner).setMarketingReward(120000000000000000000n);
  ...
  let iAddrBytes = await ethers.provider.getStorage(await cosmicGameProxy.getAddress(),'0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc');
  let iAddr = await ethers.AbiCoder.defaultAbiCoder().decode(['address'],iAddrBytes).toString();
  ...
  await cosmicGameProxy.donateWithInfo(donationData,{value: hre.ethers.parseEther('9')});
  await cosmicGameProxy.donateWithInfo(donationData,{value: hre.ethers.parseEther('8')});
  ...
  */
  ...
  await cosmicToken.connect(addr1).approve(await cosmicGameProxy.getAddress(),hre.ethers.parseEther('10000000'));
  await cosmicToken.connect(addr2).approve(await cosmicGameProxy.getAddress(),hre.ethers.parseEther('10000000'));
  await cosmicToken.connect(addr3).approve(await cosmicGameProxy.getAddress(),hre.ethers.parseEther('10000000'));
  await cosmicToken.connect(addr4).approve(await cosmicGameProxy.getAddress(),hre.ethers.parseEther('10000000'));
  await cosmicGameProxy.connect(addr1).bidWithCST("CST bid addr1")
  prizeTime = await cosmicGameProxy.timeUntilPrize();
  await ethers.provider.send("evm_increaseTime", [Number(prizeTime)]);
  await ethers.provider.send("evm_mine");
  tx = await cosmicGameProxy.connect(addr1).claimPrize({gasLimit:3000000});
