// Set numRaffleNFTWinnersPerRound 
package main
...
func main() {
	...
	if len(os.Args) < 4 {
		fmt.Printf("Usage: \n\t\t%v [priv_key] [contract_addr] [num]\n\n\t\tChanges numRaffleWinnersPerRound\n",os.Args[0])
		os.Exit(1)
	}
	...
	num_val := big.NewInt(0)
	_,success := num_val.SetString(num_str,10)
	if !success {
	...
	tx,err := cosmic_game_ctrct.SetNumRaffleNFTWinnersPerRound(txopts,num_val)
