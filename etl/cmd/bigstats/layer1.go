...
func main() {
	...
	schema_name := flag.StringVar("schema", false, "Schema name")
	...
	fname=fmt.Sprintf("%v/etl_%v_error.log",log_dir,schema_name)
	if err!=nil {
		fmt.Printf("Can't start: %v\n",err)
		os.Exit(1)
	}
	...
	if stored_chain_id != network_chain_id.Int64() {
		if stored_chain_id == 0 {
			// not initialized yet
			storage.Bitstats_set_chain_id(network_chain_id.Int64())
		} else {
			Fatalf(
