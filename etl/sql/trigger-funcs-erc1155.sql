...
BEGIN
	...
	FOREACH v_token_id IN ARRAY STRING_TO_ARRAY(NEW.token_ids,',')
		LOOP
			v_id := v_token_id::BIGINT;
			...
			INSERT INTO erc1155_bal(block_num,tx_id,parent_id,contract_aid,token_id,time_stamp,aid,amount)
					VALUES(NEW.block_num,NEW.tx_id,NEW.id,NEW.contract_aid,v_token_id,NEW.time_stamp,NEW.to_aid,NEW.amount);
			IF NEW.op_type == 1 THEN -- mint
...
BEGIN
	...
	FOREACH v_token_id IN ARRAY STRING_TO_ARRAY(OLD.token_ids,',')
		LOOP
			v_id := v_token_id::BIGINT;
			v_amount := v_amounts[v_counter]::BIGINT;
			IF OLD.op_type == 1 THEN -- mint
