// Sets the delay duration (seconds) before round activation.
// This is the inactive window opened right after claimMainPrize(), during which
// a UUPS upgrade is authorized. 120 = 2 minutes.
package main
...
func main() {
	...
	if len(os.Args) != 3 {
		cutils.PrintUsage(os.Args[0],
			"[cosmicgame_contract_addr] [seconds]",
			"Sets delayDurationBeforeRoundActivation (e.g., 120 = 2 minutes)",
	...
	if seconds < 0 {
		cutils.Fatal("Seconds must be non-negative")
	}
	...
	cutils.PrintKeyValue("Current Delay", currentDelay.String())
	cutils.PrintKeyValueDuration("Current Delay (duration)", currentDelay.Int64())

	cutils.Section("NEW VALUES")
	cutils.PrintKeyValue("New Delay", seconds)
	cutils.PrintKeyValueDuration("New Delay (duration)", seconds)
