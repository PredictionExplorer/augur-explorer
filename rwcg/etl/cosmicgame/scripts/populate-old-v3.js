/**
 * populate-old-v3.js — sample chain population for Cosmic-Signature v3 branch.
 *
 * Flow: deploy CosmicSignatureGame (V1 proxy) → run rounds on V1 APIs → upgrade
 * to CosmicSignatureGameV2 with initializeV2() → run rounds on V2 → upgrade to
 * CosmicSignatureGameV3 with reinitialize() → run rounds on V3.
 *
 * This mirrors the real on-chain history (V1 → V2 already live, V2 → V3 next), so the
 * populated DB contains BOTH MainPrizeClaimed topics:
 *   V2 topic 0x8c551ec2…  (rounds claimed before the V3 upgrade)
 *   V3 topic 0x9314e785…  (rounds claimed after; adds prizeNumCosmicSignatureNfts, multi-NFT prize)
 *
 * V3-specific behavior exercised (rounds 8+):
 *   - Multi-NFT main prize: winner receives `mainPrizeNumCosmicSignatureNfts` (default 3) sequential NFTs.
 *   - 90/10 bid CST reward split (Comment-202607161): 90% minted to the outbid bidder, ~10% to the
 *     new bidder. Rounds 8/9 space bids in time so the linear reward is non-zero and the split fires.
 *   - Late bid price premium near mainPrizeTime (BiddingV3).
 *
 ...
 * isGameV2 call shapes are reused after the V3 upgrade.
 *
 * Sample ERC20: uses Samp if present, else FuzzTestMockErc20.
 */
...
const CONFIG = {
    ...
    bids: {
        ...
        // V3 rounds (post V2→V3 upgrade). Bids are spaced in time by the V3 runner so the linear
        // bid CST reward is non-zero and the 90/10 split (outbid bidder / new bidder) is exercised.
        round8: [
            { signer: 'addr2', message: 'v3 r8 opening (addr2)', rwalkToken: -1 },
            { signer: 'addr1', message: 'v3 r8 outbid (addr1)', rwalkToken: -1 },
            { signer: 'addr3', message: 'v3 r8 CST bid (addr3)', useCst: true },
        ],
        round9: [
            { signer: 'addr1', message: 'v3 r9 opening (addr1)', rwalkToken: -1 },
            { signer: 'addr2', message: 'v3 r9 outbid (addr2)', rwalkToken: -1 },
        ],
    },
    staking: {
        rwalkTokens: {
    ...
    gameSettings: {
        ...
        bidCstRewardAmount: 130000000000000000000n,
        bidCstRewardAmountMultiplier: 130000000000000000000n,
        marketingWalletContribution: 120000000000000000000n,
        cstMinLimit: 150000000000000000000n,
        maxMessageLength: 199,
        cstDutchAuctionDurationDivisor: 13 * 3600,
        cstDutchAuctionDuration: 30 * 60,
        cstDutchAuctionDurationChangeDivisor: 250,
        // V3 (ISystemEventsV3) config — set post-upgrade to emit the 5 new *Changed events.
...
async function placeBid(signer, rwalkTokenId, message, value) {
    ...
    if (isGameV2) {
        await cosmicGameProxy.connect(signer).bidWithEth(rwalkTokenId, message, 0n, { value: bidValue });
    } else {
...
async function placeCstBid(signer, message) {
    await poolCstTo(signer);
    const cstPrice = await cosmicGameProxy.getNextCstBidPrice();
    const bal = await cosmicToken.balanceOf(signer.address);
    if (bal < cstPrice) {
        throw new Error(`Insufficient CST: have ${hre.ethers.formatEther(bal)}, bid costs ${hre.ethers.formatEther(cstPrice)}`);
    }
    if (isGameV2) {
        await cosmicGameProxy.connect(signer).bidWithCst(cstPrice, message, 0n);
    } else {
...
async function placeBidsForRound(roundName) {
    ...
    for (const bid of bids) {
        ...
        if (bid.useCst) {
            ...
            if (isGameV2) {
                await cosmicGameProxy.connect(signer).bidWithEth(rwalkToken, bid.message, 0n, { value: bidValue });
            } else {
...
async function donateNft(signer, nftContract, tokenId, bidMessage) {
    ...
    if (isGameV2) {
        await cosmicGameProxy.connect(signer).bidWithEthAndDonateNft(
            -1, bidMessage, 0n, nftAddr, tokenId, { value: bidPrice },
        );
    } else {
        await cosmicGameProxy.connect(signer).bidWithEthAndDonateNft(
            -1, bidMessage, nftAddr, tokenId, { value: bidPrice },
        );
    }
}
...
async function donateErc20Token(tokenContract, amount, bidMessage) {
    ...
    if (isGameV2) {
        await cosmicGameProxy.bidWithEthAndDonateToken(-1, bidMessage, 0n, tokenAddr, amount, { value: bidPrice });
    } else {
...
async function poolCstTo(signer) {
    for (const donorKey of ["addr1", "addr2", "addr4", "owner"]) {
        const donor = signerMap[donorKey] || owner;
        if (donor.address === signer.address) continue;
        const donorBal = await cosmicToken.balanceOf(donor.address);
        if (donorBal > 0n) {
            await cosmicToken.connect(donor).transfer(signer.address, donorBal);
        }
    }
}

/** Deploy two sample ERC20s for donation tests (Samp on older branches, FuzzTestMockErc20 on v2). */
async function deploySampleErc20Tokens(deployer) {
    let factory;
    let contractName;
    try {
        factory = await hre.ethers.getContractFactory("Samp");
        contractName = "Samp";
    } catch (_) {
        factory = await hre.ethers.getContractFactory("FuzzTestMockErc20");
        contractName = "FuzzTestMockErc20";
    }

    const samp1 = await factory.connect(deployer).deploy();
    await samp1.waitForDeployment();
    const samp2 = await factory.connect(deployer).deploy();
    await samp2.waitForDeployment();

    if (contractName === "FuzzTestMockErc20") {
        const mintAmount = hre.ethers.parseEther("100000000000");
        await samp1.connect(deployer).mint(deployer.address, mintAmount);
        await samp2.connect(deployer).mint(deployer.address, mintAmount);
    }

    console.log(`Deployed sample ERC20 tokens (${contractName})`);
    return { samp1, samp2 };
}
...
/** _authorizeUpgrade requires an inactive round; claimMainPrize sets activation ~now+delay. */
async function deferRoundActivationForUpgrade() {
    const latestBlock = await hre.ethers.provider.getBlock("latest");
    const futureActivation = BigInt(latestBlock.timestamp) + 86400n;
    await (await cosmicGameProxy.connect(owner).setRoundActivationTime(futureActivation)).wait();
    console.log(`Deferred round activation to ${futureActivation} for proxy upgrade`);
}
...
async function setupGameConfiguration() {
    ...
    await cosmicGameProxy.connect(owner).setTimeoutDurationToClaimMainPrize(gameSettings.timeoutClaimPrize, g);
    await cosmicGameProxy.connect(owner).setMainPrizeTimeIncrementInMicroSeconds(gameSettings.prizeTimeIncrement, g);
    await cosmicGameProxy.connect(owner).setInitialDurationUntilMainPrizeDivisor(gameSettings.initialDurationDivisor, g);
    await cosmicGameProxy.connect(owner).setNumRaffleEthPrizesForBidders(gameSettings.numRaffleEthPrizes, g);
    await cosmicGameProxy.connect(owner).setNumRaffleCosmicSignatureNftsForBidders(gameSettings.numRaffleNftsBidders, g);
    await cosmicGameProxy.connect(owner).setNumRaffleCosmicSignatureNftsForRandomWalkNftStakers(gameSettings.numRaffleNftsStakers, g);
    await cosmicGameProxy.connect(owner).setMainEthPrizeAmountPercentage(gameSettings.mainPrizePercentage, g);
    await cosmicGameProxy.connect(owner).setCharityEthDonationAmountPercentage(gameSettings.charityPercentage, g);
    await cosmicGameProxy.connect(owner).setRaffleTotalEthPrizeAmountForBiddersPercentage(gameSettings.rafflePercentage, g);
    await cosmicGameProxy.connect(owner).setChronoWarriorEthPrizeAmountPercentage(gameSettings.chronoPercentage, g);
    await cosmicGameProxy.connect(owner).setCosmicSignatureNftStakingTotalEthRewardAmountPercentage(gameSettings.stakingPercentage, g);
    await cosmicGameProxy.connect(owner).setCstPrizeAmount(gameSettings.cstPrizeAmount, g);
    // V2 branch V1 API: BidCstRewardAmountChanged (not CstRewardAmountForBiddingChanged)
    await cosmicGameProxy.connect(owner).setBidCstRewardAmount(gameSettings.bidCstRewardAmount, g);
    await cosmicGameProxy.connect(owner).setMarketingWalletCstContributionAmount(gameSettings.marketingWalletContribution, g);
    await cosmicGameProxy.connect(owner).setCstDutchAuctionBeginningBidPriceMinLimit(gameSettings.cstMinLimit, g);
...
async function upgradeGameProxyToV2() {
    ...
    const roundBefore = await cosmicGameProxy.roundNum();
    console.log(`Upgrading proxy ${proxyAddr} to CosmicSignatureGameV2 (roundNum=${roundBefore})...`);

    const CosmicSignatureGameV2 = await hre.ethers.getContractFactory("CosmicSignatureGameV2", owner);
    cosmicGameProxy = await hre.upgrades.upgradeProxy(
        cosmicGameProxy,
        CosmicSignatureGameV2,
        {
            kind: "uups",
            // The v3 branch renamed the V2 initializer initializeV2() → reinitialize() (reinitializer(2));
            // every version now shares the name declared in CosmicSignatureGameV2Base.
            call: "reinitialize",
            // V2 reuses V1 slots with renamed fields; new field occupies former __gap slot
    ...
    isGameV2 = true;
    implementationAddr = await hre.upgrades.erc1967.getImplementationAddress(proxyAddr);
    console.log(`V2 upgrade complete, implementation=${implementationAddr}`);
}

async function upgradeGameProxyToV3() {
    const proxyAddr = await cosmicGameProxy.getAddress();
    const roundBefore = await cosmicGameProxy.roundNum();
    console.log(`Upgrading proxy ${proxyAddr} to CosmicSignatureGameV3 (roundNum=${roundBefore})...`);

    const CosmicSignatureGameV3 = await hre.ethers.getContractFactory("CosmicSignatureGameV3", owner);
    cosmicGameProxy = await hre.upgrades.upgradeProxy(
        cosmicGameProxy,
        CosmicSignatureGameV3,
        {
            kind: "uups",
            // reinitializer(3): sets the 5 new V3 params to their defaults (see docs/v3-vs-v2-changes.md).
            call: "reinitialize",
            // The real V2→V3 upgrade needs NO unsafe flags (append-only layout). We keep them here only
            // because this sample forced unsafeSkipStorageCheck during the earlier V1→V2 upgrade.
            unsafeAllowRenames: true,
            unsafeSkipStorageCheck: true,
        },
    );
    ...
    isGameV3 = true;
    implementationAddr = await hre.upgrades.erc1967.getImplementationAddress(proxyAddr);
    console.log(`V3 upgrade complete, implementation=${implementationAddr}`);
}

/**
 * V3-only config admin events (ISystemEventsV3 → cg_adm_* record types 40–44).
 * Emits:
 ...
 * (call it right after upgradeGameProxyToV3(), before setActivationTimeToNow()).
 */
async function emitV3ConfigAdminEvents() {
    if (!isGameV3) {
        throw new Error('emitV3ConfigAdminEvents requires V3 implementation');
    }
    const { gameSettings } = CONFIG;
    const g = { gasLimit: 1000000 };
    const game = cosmicGameProxy.connect(owner);
    const roundNum = await cosmicGameProxy.roundNum();
    console.log(`\n=== V3 config admin events (roundNum=${roundNum}) ===`);

    await game.setRoundLateBidDurationDivisor(gameSettings.roundLateBidDurationDivisor, g);
    console.log(`  setRoundLateBidDurationDivisor → RoundLateBidDurationDivisorChanged: ${gameSettings.roundLateBidDurationDivisor}`);

    await game.setRoundLateBidPricePremiumAmountBaseMultiplier(gameSettings.roundLateBidPricePremiumAmountBaseMultiplier, g);
    console.log(`  setRoundLateBidPricePremiumAmountBaseMultiplier → RoundLateBidPricePremiumAmountBaseMultiplierChanged: ${gameSettings.roundLateBidPricePremiumAmountBaseMultiplier}`);
...
/**
 * Runs one V3 round from CONFIG.bids[roundName], spacing bids in time so the linear bid CST reward
 * (bidCstRewardAmountPerMinute) is non-zero and the 90/10 split fires (needs >= 2 bidders). Then claims
 * and reports how many Cosmic Signature NFTs the winner received (mainPrizeNumCosmicSignatureNfts).
 */
async function runV3Round(roundName, claimer) {
    const bids = CONFIG.bids[roundName];
    console.log(`\n=== ${roundName.toUpperCase()} (V3) === placing ${bids.length} spaced bids`);
    ...
    for (let i = 0; i < bids.length; i++) {
        ...
        const signer = signerMap[bid.signer];
        if (i > 0) {
            // Space bids so the linear reward accrues and the outbid bidder gets the 90% share.
            await ethers.provider.send("evm_increaseTime", [CONFIG.cstBidTimeSkipSec]);
            await ethers.provider.send("evm_mine");
        }
        if (bid.useCst) {
            await poolCstTo(signer);
            ...
            await placeBid(signer, bid.rwalkToken, bid.message);
        }
    }

    const totalSupplyBefore = await cosmicSignature.totalSupply();
    // Higher gas limit: the V3 main prize mints mainPrizeNumCosmicSignatureNfts NFTs (default 3) plus all secondary prizes.
    const receipt = await advanceTimeAndClaim(claimer, 30000000);

    // Report the ACTUAL main-prize NFT count from the V3 MainPrizeClaimed event (prizeNumCosmicSignatureNfts).
    // Note: the winner may also win raffle/endurance/chrono NFTs in the same claim, so counting every
    // NftMinted to the winner would over-report; we read the authoritative field instead.
    const mainPrizeClaimedSig = cosmicGameProxy.interface.getEvent("MainPrizeClaimed").topicHash;
    let mainPrizeNfts = null;
    let firstNftId = null;
    for (const log of receipt.logs) {
        if (log.topics[0] !== mainPrizeClaimedSig) continue;
        const args = cosmicGameProxy.interface.parseLog(log).args.toObject();
        mainPrizeNfts = args.prizeNumCosmicSignatureNfts;
        firstNftId = args.prizeFirstCosmicSignatureNftId;
        break;
    }
    const totalSupplyAfter = await cosmicSignature.totalSupply();
    console.log(`  ${roundName}: winner ${claimer.address.substring(0,10)}… main-prize NFTs=${mainPrizeNfts} (first id=${firstNftId}); total NFTs minted this claim (all prizes)=${totalSupplyAfter - totalSupplyBefore}`);
    await stakeAvailableNfts();
}

/** Nudge a numeric admin field by ~1% (minimum +1) so the change event fires. */
function bumpOnePercent(value) {
    if (value === 0n) return 1n;
    const bumped = (value * 101n) / 100n;
    return bumped > value ? bumped : value + 1n;
}

/** Small +1 bump for percentage fields (stay within typical 0–100 range). */
function bumpSmallPercent(value) {
...
async function emitAllV1AdminEvents() {
    ...
    console.log("Emitting V1-era admin events (pre-upgrade)...");

    await game.setDelayDurationBeforeRoundActivation(
        bumpOnePercent(await cosmicGameProxy.delayDurationBeforeRoundActivation()), g);
    await game.setRoundActivationTime(
        (await cosmicGameProxy.roundActivationTime()) + 3600n, g);
    await game.setEthDutchAuctionDurationDivisor(
        bumpOnePercent(await cosmicGameProxy.ethDutchAuctionDurationDivisor()), g);
    await game.setEthDutchAuctionEndingBidPriceDivisor(
        bumpOnePercent(await cosmicGameProxy.ethDutchAuctionEndingBidPriceDivisor()), g);
    await game.setEthBidPriceIncreaseDivisor(
    ...
    await game.setChronoWarriorEthPrizeAmountPercentage(
        bumpSmallPercent(await cosmicGameProxy.chronoWarriorEthPrizeAmountPercentage()), g);
    await game.setRaffleTotalEthPrizeAmountForBiddersPercentage(
        bumpSmallPercent(await cosmicGameProxy.raffleTotalEthPrizeAmountForBiddersPercentage()), g);
    await game.setNumRaffleEthPrizesForBidders(
        (await cosmicGameProxy.numRaffleEthPrizesForBidders()) + 1n, g);
    await game.setNumRaffleCosmicSignatureNftsForBidders(
        (await cosmicGameProxy.numRaffleCosmicSignatureNftsForBidders()) + 1n, g);
    await game.setNumRaffleCosmicSignatureNftsForRandomWalkNftStakers(
        (await cosmicGameProxy.numRaffleCosmicSignatureNftsForRandomWalkNftStakers()) + 1n, g);
    await game.setCosmicSignatureNftStakingTotalEthRewardAmountPercentage(
        bumpSmallPercent(await cosmicGameProxy.cosmicSignatureNftStakingTotalEthRewardAmountPercentage()), g);
    await game.setInitialDurationUntilMainPrizeDivisor(
        bumpOnePercent(await cosmicGameProxy.initialDurationUntilMainPrizeDivisor()), g);
    await game.setMainPrizeTimeIncrementInMicroSeconds(
        bumpOnePercent(await cosmicGameProxy.mainPrizeTimeIncrementInMicroSeconds()), g);
    await game.setMainPrizeTimeIncrementIncreaseDivisor(
        bumpOnePercent(await cosmicGameProxy.mainPrizeTimeIncrementIncreaseDivisor()), g);
    await game.setTimeoutDurationToClaimMainPrize(
        bumpOnePercent(await cosmicGameProxy.timeoutDurationToClaimMainPrize()), g);
    await game.setMainEthPrizeAmountPercentage(
        bumpSmallPercent(await cosmicGameProxy.mainEthPrizeAmountPercentage()), g);
    await game.setMarketingWalletCstContributionAmount(
        bumpOnePercent(await cosmicGameProxy.marketingWalletCstContributionAmount()), g);
    await game.setCharityEthDonationAmountPercentage(
        bumpSmallPercent(await cosmicGameProxy.charityEthDonationAmountPercentage()), g);

    await game.setCosmicSignatureToken(await cosmicToken.getAddress(), g);
    await game.setRandomWalkNft(await randomWalkNFT.getAddress(), g);
    await game.setCosmicSignatureNft(await cosmicSignature.getAddress(), g);
    await game.setPrizesWallet(await prizesWallet.getAddress(), g);
    await game.setStakingWalletRandomWalkNft(await stakingWalletRandomWalkNft.getAddress(), g);
    await game.setStakingWalletCosmicSignatureNft(await stakingWalletCosmicSignatureNft.getAddress(), g);
    await game.setMarketingWallet(await marketingWallet.getAddress(), g);
    await game.setCharityAddress(await charityWallet.getAddress(), g);

    console.log("V1-era admin events emitted");
}

async function emitAllV2AdminEvents() {
    const g = { gasLimit: 1000000 };
    const game = cosmicGameProxy.connect(owner);

    console.log("Emitting V2 admin events (post-upgrade)...");
    ...
    await game.setDelayDurationBeforeRoundActivation(
        bumpOnePercent(await cosmicGameProxy.delayDurationBeforeRoundActivation()), g);
    await game.setRoundActivationTime(
        (await cosmicGameProxy.roundActivationTime()) + 7200n, g);
    await game.setEthDutchAuctionDurationDivisor(
        (await cosmicGameProxy.ethDutchAuctionDurationDivisor()) + 11n, g);
    await game.setEthDutchAuctionEndingBidPriceDivisor(
        (await cosmicGameProxy.ethDutchAuctionEndingBidPriceDivisor()) + 40n, g);
    await game.setEthBidPriceIncreaseDivisor(
        bumpOnePercent(await cosmicGameProxy.ethBidPriceIncreaseDivisor()), g);
    await game.setEthBidRefundAmountInGasToSwallowMaxLimit(
        bumpOnePercent(await cosmicGameProxy.ethBidRefundAmountInGasToSwallowMaxLimit()), g);
    ...
    // Edge case: re-set unchanged values (still emits events)
    await game.setStakingWalletCosmicSignatureNft(await stakingWalletCosmicSignatureNft.getAddress(), g);
    await game.setStakingWalletRandomWalkNft(await stakingWalletRandomWalkNft.getAddress(), g);

    const mainPrizeIncreaseDivisor = await cosmicGameProxy.mainPrizeTimeIncrementIncreaseDivisor();
    await game.setMainPrizeTimeIncrementIncreaseDivisor(mainPrizeIncreaseDivisor, g);

    const timeoutDuration = await cosmicGameProxy.timeoutDurationToClaimMainPrize();
    await game.setTimeoutDurationToClaimMainPrize(timeoutDuration, g);
    const mainPrizeTimeout = Number(timeoutDuration);

    const bidPriceIncrease = await cosmicGameProxy.ethBidPriceIncreaseDivisor();
    await game.setEthBidPriceIncreaseDivisor(bidPriceIncrease, g);

    const prizeTimeIncrement = await cosmicGameProxy.mainPrizeTimeIncrementInMicroSeconds();
    await game.setMainPrizeTimeIncrementInMicroSeconds(prizeTimeIncrement, g);

    const initialDuration = await cosmicGameProxy.initialDurationUntilMainPrizeDivisor();
    await game.setInitialDurationUntilMainPrizeDivisor(initialDuration, g);

    const delayDuration = await cosmicGameProxy.delayDurationBeforeRoundActivation();
    await game.setDelayDurationBeforeRoundActivation(delayDuration, g);
    ...
    const charityReceiver = await charityWallet.charityAddress();
    await charityWallet.connect(owner).setCharityAddress(charityReceiver);
    ...
    console.log("V2 admin events emitted");
    return mainPrizeTimeout;
}
...
async function main() {
    ...
    const { samp1, samp2 } = await deploySampleErc20Tokens(owner);
    ...
    const cstSkip = CONFIG.cstBidTimeSkipSec;

    await approveCosmicToken([addr1, addr2, addr3, addr4]);
    await poolCstTo(addr3);

    // Bid with CST
    await ethers.provider.send("evm_increaseTime", [cstSkip]);
    ...
    await poolCstTo(addr3);
    const cstPriceForNftCombo = await cosmicGameProxy.getNextCstBidPrice();
    const addr3BalForNftCombo = await cosmicToken.balanceOf(addr3.address);
    if (addr3BalForNftCombo >= cstPriceForNftCombo) {
        await cosmicGameProxy.connect(addr3).bidWithCstAndDonateNft(
            cstPriceForNftCombo * 2n,
        ...
        console.warn(`Skipping CST+NFT combo: have ${hre.ethers.formatEther(addr3BalForNftCombo)} CST, need ${hre.ethers.formatEther(cstPriceForNftCombo)}`);
    }
    ...
    const mainPrizeTimeout = await emitAllV2AdminEvents();
    ...
    // === V2 → V3 UPGRADE ===
    // The upgrade authorization requires an inactive round (_authorizeUpgrade). Defer round activation,
    // upgrade to V3 (reinitialize sets the 5 new params), emit the V3 config events while still inactive,
    // then run V3 rounds that exercise the multi-NFT main prize (V3 MainPrizeClaimed topic) and the
    // 90/10 bid CST reward split.
    await deferRoundActivationForUpgrade();
    await upgradeGameProxyToV3();
    await emitV3ConfigAdminEvents();

    await runV3Round('round8', addr3);
    await runV3Round('round9', addr2);
    // === END V2 → V3 UPGRADE ===
    ...
    // Re-print addresses so the final (V3) implementation address is captured for cg_contracts.
    console.log("Final contract addresses (post V3 upgrade):");
    printContractAddresses();
