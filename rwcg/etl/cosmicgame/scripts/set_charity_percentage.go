// Sets charityEthDonationAmountPercentage (percentage of funds donated to charity)
package main
...
func main() {
	...
	currentValue, err := cosmicGame.CharityEthDonationAmountPercentage(copts)
	if err != nil {
		cutils.Fatal("Error getting current value: %v", err)
	}

	cutils.Section("CHARITY DONATION PERCENTAGE CONFIG")
	cutils.PrintKeyValue("Current Value", currentValue.String()+"%")
	cutils.PrintKeyValue("New Value", percentageVal.String()+"%")

	// Create and submit transaction
	cutils.PrintTxSubmitting("SetCharityEthDonationAmountPercentage", nil, cutils.GasLimitAdminCall, net.GasPrice)

	txopts := cutils.CreateTransactOpts(net, acc, nil, cutils.GasLimitAdminCall)

	tx, err := cosmicGame.SetCharityEthDonationAmountPercentage(txopts, percentageVal)
