...
func main() {
	...
	if len(os.Args) != 3 {
		cutils.PrintUsage(os.Args[0],
			...
			"Accepts a buy or sell offer (purchases token on the marketplace).",
	...
	cutils.Section("ACCEPT OFFER INFO")
	...
	cutils.PrintTxSubmitting("Accept"+offerType+"Offer", value, gasLimit, cutils.AdjustGasPrice(net.GasPrice))

	var tx *types.Transaction
	if isSellOffer {
		tx, err = marketCtrct.AcceptSellOffer(txopts, big.NewInt(offerID))
	} else {
		tx, err = marketCtrct.AcceptBuyOffer(txopts, big.NewInt(offerID))
	}
