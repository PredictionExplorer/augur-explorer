...
func (sw *SQLStorageWrapper) contractAidForCorrection(meta *AdminCorrectionMeta, contractAddr string, blockNum, txId int64) int64 {
	if contractAddr == "" {
		return meta.ContractAid
	}
	return sw.S.Lookup_or_create_address(contractAddr, blockNum, txId)
}
...
func (sw *SQLStorageWrapper) Insert_admin_correction_decimal_row_for_contract(table, column, value string, meta *AdminCorrectionMeta, contractAddr string) error {
	contractAid := sw.contractAidForCorrection(meta, contractAddr, meta.BlockNum, meta.TxId)
	query := fmt.Sprintf(
