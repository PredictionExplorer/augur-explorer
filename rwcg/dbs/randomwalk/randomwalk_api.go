...
func (sw *SQLStorageWrapper) Get_minted_tokens_sequentially(rwalk_aid int64,offset,limit int) []p.RW_API_TokenMint {
	...
	rows,err := sw.S.Db().Query(query,rwalk_aid,offset,limit)
	if (err!=nil) {
		sw.S.Log_msg(fmt.Sprintf("DB error: %v (query=%v)",err,query))
		os.Exit(1)
	}
...
func (sw *SQLStorageWrapper) Get_random_walk_stats(rwalk_aid int64) p.RW_API_RWalkStats {
	...
	res := sw.S.Db().QueryRow(query,rwalk_aid)
	err := res.Scan(
		&output.TradingVol,
		&output.NumTrades,
		&output.TokensMinted,
...
func (sw *SQLStorageWrapper) Get_market_stats(market_aid int64) p.RW_API_MarketStats {
	...
	res := sw.S.Db().QueryRow(query,market_aid)
	err := res.Scan(
		&output.TradingVol,
		&output.NumTrades,
	)
...
func (sw *SQLStorageWrapper) Get_market_trading_volume_by_period(contract_aid int64,init_ts int,fin_ts int,interval int) []p.RW_API_RandomWalkVolumeHistory {
	...
	if (err!=nil) {
		if err != sql.ErrNoRows {
			sw.S.Log_msg(fmt.Sprintf("Error in Get_randomwalk_trading_volume_by_period(): %v, q=%v",err,query))
			os.Exit(1)
		}
	}
	...
	for rows.Next() {
		...
		if err!=nil {
			sw.S.Log_msg(fmt.Sprintf("DB error: %v",err))
			os.Exit(1)
		}
		if sum_volume.Valid {
...
func (sw *SQLStorageWrapper) Get_floor_price(rwalk_aid int64,market_aid int64) (no_offers bool,floor_price float64,offer_id int64,token_id int64,err error) {
...
func (sw *SQLStorageWrapper) Get_rwalk_user_info(user_aid int64,rwalk_aid int64) (p.RW_API_UserInfo,error) {
	...
	res := sw.S.Db().QueryRow(query,user_aid)
	...
	res = sw.S.Db().QueryRow(query,user_aid,rwalk_aid)
	err = res.Scan(
			&output.TotalVolume,
...
func (sw *SQLStorageWrapper) Get_rwalk_mint_intervals(rwalk_aid int64) []p.RW_API_MintInterval {
	...
	rows,err := sw.S.Db().Query(query,rwalk_aid)
	if (err!=nil) {
		sw.S.Log_msg(fmt.Sprintf("DB error: %v (query=%v)",err,query))
		os.Exit(1)
	}
