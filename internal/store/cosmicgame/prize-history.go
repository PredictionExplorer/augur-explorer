...
// PrizeHistoryByUser returns the detailed prize history of one winner across
...
func (r *Repo) PrizeHistoryByUser(ctx context.Context, winnerAid int64, offset, limit int) ([]p.CGPrizeHistory, error) {
	query := `SELECT
		...
		FROM cg_prize_withdrawal pw
			JOIN transaction t ON t.id = pw.tx_id
		...
		SELECT
			...
			TRUE AS claimed,
			TRUE AS is_timeout_claim
		FROM cg_donated_nft_claimed nc
			JOIN transaction t ON t.id = nc.tx_id
			JOIN cg_prize_claim pc ON pc.round_num = nc.round_num
			JOIN address wa_token ON nc.token_aid = wa_token.address_id
		...
		SELECT
			...
			TRUE AS claimed,
			TRUE AS is_timeout_claim
		FROM cg_donated_tok_claimed tc
			JOIN transaction t ON t.id = tc.tx_id
			JOIN cg_prize_claim pc ON pc.round_num = tc.round_num
			JOIN address wa_token ON tc.token_aid = wa_token.address_id
	...
	return queryList(ctx, r, "prize history by user", 32, query, scan, winnerAid, offset, limit)
}
...
func (r *Repo) ClaimHistoryGlobal(ctx context.Context, offset, limit int) ([]p.CGPrizeHistory, error) {
	query := `SELECT
		...
		FROM cg_prize_withdrawal pw
			JOIN transaction t ON t.id = pw.tx_id
			JOIN address wa_winner ON pw.winner_aid = wa_winner.address_id
		...
		SELECT
			...
			TRUE AS claimed,
			TRUE AS is_timeout_claim,
			wa_beneficiary.addr AS winner_addr,
			nc.winner_aid
		FROM cg_donated_nft_claimed nc
			JOIN transaction t ON t.id = nc.tx_id
			JOIN cg_prize_claim pc ON pc.round_num = nc.round_num
			JOIN address wa_token ON nc.token_aid = wa_token.address_id
			JOIN address wa_beneficiary ON nc.winner_aid = wa_beneficiary.address_id
		...
		FROM cg_donated_tok_claimed tc
			...
			JOIN cg_prize_claim pc ON pc.round_num = tc.round_num
			JOIN address wa_token ON tc.token_aid = wa_token.address_id
			JOIN address wa_beneficiary ON tc.winner_aid = wa_beneficiary.address_id
	...
	return queryList(ctx, r, "claim history global", 32, query, scan, offset, limit)
}
