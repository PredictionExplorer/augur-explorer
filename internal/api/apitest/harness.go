...
func newHarness(ctx context.Context, db *testdb.DB) (*harness, error) {
	...
	// build a keyless router next to this one. StartTraitIngestion is
	// deliberately not called: a live ingest loop would race the fixtures,
	// and the trait tests write the rows they need directly.
