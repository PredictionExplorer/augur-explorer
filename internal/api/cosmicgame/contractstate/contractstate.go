...
type DataSource interface {
	...
	// LastCstBidEvtlogForBidder returns the event log id of the bidder's
	// latest CST bid in the round.
	LastCstBidEvtlogForBidder(ctx context.Context, roundNum int64, bidderAddr string) (int64, error)
}
...
// mirror the three refresh cycles; the zero value of each field is what
...
// Run drives the three periodic refresh loops until ctx is cancelled. The
...
func (s *State) Run(ctx context.Context) {
	...
	go loop(s.variablesInterval, s.refreshVariables)
