...
func update_profit_ranks(records []RankStats) {
	...
	for i:=0 ; i < num_recs  ; i++ {
		...
		storage.Update_polymarket_top_profit_rank(record.Aid,rank_value,record.ProfitLoss)
	}
}
func update_trade_ranks(records []RankStats) {
	...
	for i:=0 ; i < num_recs  ; i++ {
		record := &records[i]
		rank_value := (float64(i)/float64(num_recs))*100.0 + 1.0
		storage.Update_polymarket_top_total_trades_rank(record.Aid,rank_value,record.TotalTrades)
	}
}
func update_volume_ranks(records []RankStats) {
	...
	for i:=0 ; i < num_recs  ; i++ {
		record := &records[i]
		rank_value := (float64(i)/float64(num_recs))*100.0 + 1.0
		storage.Update_polymarket_top_volume_rank(record.Aid,rank_value,record.VolumeTraded)
	}
}
func main() {
	// Design Notes:
	...
	records := storage.Get_polymarket_ranking_data_for_all_users()
