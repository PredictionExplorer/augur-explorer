...
func main() {
	...
	if len(os.Args) != 4 {
		fmt.Printf("Usage: \n\t\t%v [priv_key] [contract_addr] [seconds]\n\n\t\tSet activation interval for next round\n",os.Args[0])
		os.Exit(1)
	}
	...
	if err != nil {
		fmt.Printf("error parsing seconds parameter: %v\n",err)
		os.Exit(1)
	}
	...
	tx,err := cosmic_game_ctrct.SetDelayDurationBeforeRoundActivation(txopts,big.NewInt(seconds))
