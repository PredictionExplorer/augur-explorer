...
func proc_pool_created(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"PoolCreated",log.Data)
	if err != nil {
...
func proc_liquidity_changed(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"LiquidityChanged",log.Data)
	if err != nil {
...
func proc_price_market(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"PriceMarketCreated",log.Data)
	if err != nil {
...
func proc_sports_market(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"SportsMarketCreated",log.Data)
	if err != nil {
...
func proc_trusted_market(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"TrustedMarketCreated",log.Data)
	if err != nil {
...
func proc_shares_minted(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"SharesMinted",log.Data)
	if err != nil {
...
func proc_shares_burned(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"SharesBurned",log.Data)
	if err != nil {
...
func proc_shares_swapped(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"SharesSwapped",log.Data)
	if err != nil {
...
func proc_settlement_fee_claimed(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"SettlementFeeClaimed",log.Data)
	if err != nil {
...
func proc_protocol_fee_claimed(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"ProtocolFeeClaimed",log.Data)
	if err != nil {
		Error.Printf("Error unpacking SettlementFeeClaimed event: %v\n",err)
		os.Exit(1)
	}
...
func proc_protocol_changed(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"ProtocolChanged",log.Data)
	if err != nil {
...
func proc_protocol_fee_changed(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"ProtocolFeeChanged",log.Data)
	if err != nil {
...
func proc_settlement_fee_changed(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"SettlementFeeChanged",log.Data)
	if err != nil {
...
func proc_staker_fee_changed(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"StakerFeeChanged",log.Data)
	if err != nil {
...
func proc_winnings_claimed(log *types.Log,elog *EthereumEventLog) {
	...
	eth_evt.Receiver = common.BytesToAddress(log.Topics[1][12:])
	err := aa_abi.UnpackIntoInterface(&eth_evt,"WinningsClaimed",log.Data)
	if err != nil {
...
func proc_market_resolved(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"MarketResolved",log.Data)
	if err != nil {
...
func proc_erc20_transfer(log *types.Log,elog *EthereumEventLog) {
	...
	err := aa_abi.UnpackIntoInterface(&eth_evt,"Transfer",log.Data)
	if err != nil {
