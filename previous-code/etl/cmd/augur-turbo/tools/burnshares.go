// Withdraws collateral from market/outcome (same as burning shares)
package main
...
func main() {
	...
	if len(os.Args) < 4 {
		fmt.Printf(
			"Usage: \n\t\t%v [private_key] [market_factory] [market_id] [amount_to_withdraw]\n\t\t"+
	...
	withdraw_amount_str := os.Args[4]
	withdraw_amount := big.NewInt(0)
	_,success := withdraw_amount.SetString(withdraw_amount_str,10)
	if !success {
		fmt.Printf("Couldn't parse withdrawal amount, bad integer\n")
		os.Exit(1)
	}
	...
	fmt.Printf("Burn chares, market_id=%v\n",big_market_id.String())
	...
	tx,err := market_factory.BurnShares(txopts,big_market_id,withdraw_amount,from_address)
	if err!=nil {
