...
func process_block(bnum int64,update_last_block bool,no_chainsplit_check bool) error {
	...
	for i,tx := range transactions {
		...
		Info.Printf("Getting receipt for tx hash %v\n",tx.TxHash)
	...
	for tnum,agtx := range transactions {
		...
		Info.Printf("Waiting for receipt number %v ... \n",tnum)
		for {
			if receipt_calls[tnum] != nil {
				Info.Printf("Receipt arrived: %+v\n",receipt_calls[tnum])
		...
		Info.Printf("\t from=%v, status=%v\n",agtx.From,rcpt.Status)
		...
		if rcpt.BlockNumber.Int64() != bnum {
			...
			// currently disabled until Arbitrum solves the bug
			//storage.Block_delete_with_everything(bnum)
