...
func main() {
	...
	if len(os.Args) != 5 {
		fmt.Printf("Usage: \n\t\t%v [priv_key] [token_addr] [spender] [amount]\n\n\t\tSets allowance for a spender\n",os.Args[0])
		os.Exit(1)
	}
	...
	allowance_amount := big.NewInt(0)
	_,success := allowance_amount.SetString(amount_str,10)
	if !success {
	...
	tx,err := erc20_token.Approve(txopts,spender_addr,allowance_amount)
	if err!=nil {
