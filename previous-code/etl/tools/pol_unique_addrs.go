package main
/// Updates number of unique addresses per day for Polymarkets
import (
	"os"
	"log"
	"time"
...
func main() {	// returns 0 - no changes, 2 - day was added
	...
	for days_back > 0 {
		num_addrs,num_funders,num_traders,no_rows := storage.Calc_polymarkets_unique_addresses(day_ts,next_day_ts)
		if !no_rows {
			storage.Update_polymarkets_unique_addresses(day_ts,num_addrs,num_funders,num_traders)
		}
		tm := time.Unix(day_ts, 0)
		Info.Printf(
			"Day %v processed: from %v to %v (%v %v %v)\n",
			tm,day_ts,next_day_ts,num_addrs,num_funders,num_traders,
		)
