...
| `DB_L1EVT1_NAME`/`_HOST`/`_DBNAME`/`_USER`/`_PASS`/`_TABLE`/`_COLUMN`, `DB_L1EVT2_*` | Event tables: a status column must increase |
...
| `ANOMALY_SSH_USER`/`ANOMALY_SSH_HOST`/`ANOMALY_REMOTE_FILE`/`ANOMALY_TITLE` | Web-server anomaly file fetched via scp (produced by `loganomaly`) |
