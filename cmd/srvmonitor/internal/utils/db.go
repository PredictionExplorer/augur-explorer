// Package utils provides database connection helpers for srvmonitor.
package utils
...
func ConnectPostgres(cfg types.DatabaseConfig) (*sql.DB, error) {
	...
	if err != nil {
		_ = dbobj.Close() // best-effort cleanup if we fail to set timezone
...
// ConnectPostgresEventTable connects to a PostgreSQL database using EventTableConfig
func ConnectPostgresEventTable(cfg types.EventTableConfig) (*sql.DB, error) {
	host, port, err := net.SplitHostPort(cfg.Host)
	if err != nil {
		host = cfg.Host
		port = "5432"
