...
import (
	...
	"github.com/PredictionExplorer/augur-explorer/cmd/srvmonitor/internal/utils"
)
...
func (m *EventTableMonitor) checkDatabase(status *EventTableStatus, wg *sync.WaitGroup, errorChan chan<- string) {
	...
	defer func() { _ = dbobj.Close() }() // best-effort close of per-check connection

	query := fmt.Sprintf("SELECT %s FROM %s LIMIT 1", status.Config.ColumnName, status.Config.TableName) //nolint:gosec // G201: column/table names come from operator-provided monitor config
