...
func (d *TermboxDisplay) Clear() {
	...
	_ = termbox.Clear(termbox.ColorDefault, termbox.ColorDefault) // termbox.Clear never fails after successful Init
}
...
func (d *TermboxDisplay) Flush() {
	d.mutex.Lock()
	defer d.mutex.Unlock()
	_ = termbox.Flush() // best-effort: a failed flush only skips one screen refresh
}
