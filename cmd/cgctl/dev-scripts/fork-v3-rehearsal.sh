...
# Pure bash + curl + sed (+ cgctl for the claim/bid steps). No python/jq required.
#
...
#   claim        claim the main prize with hardhat account #0 via `cgctl claim-prize` (steps 5-6)
...
#   CGCTL             cgctl binary             (default: cgctl on PATH; build with
...
claim) # ---- steps 5-6: claim with hardhat account #0; round goes inactive ----
	PKEY_HEX=$ACCOUNT0_PKEY "$CGCTL" claim-prize "$GAME_ADDR"
	echo
...
new-round) # ---- step 8: pass the activation time and place the first gesture ----
	...
	PKEY_HEX=$ACCOUNT0_PKEY "$CGCTL" bid "$GAME_ADDR"
	echo
