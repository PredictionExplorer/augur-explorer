...
#
# Token totals and seeds come from the database via `cgctl total-tokens` and
# `cgctl token-seed` (PGSQL_* env vars required). Requires cgctl on PATH (or
# set CGCTL): go build -o cgctl ./cmd/cgctl
# Prefer imgcheck_api.sh when the public API is available (no DB needed).
...
if ! command -v "$CGCTL" >/dev/null 2>&1; then
	echo "Error: cgctl not found; build it with: go build -o cgctl ./cmd/cgctl (or set CGCTL)"
	exit 1
fi
...
TOTAL_TOKENS="$($CGCTL total-tokens)"
