...
func main() {
	...
	if *errorLogPath != "" {
		...
		defer func() { _ = errorLogger.Close() }() // best-effort close of the error log
...
func parallelScan(reader *freezerscanner.ParallelReader, start, end uint64, numWorkers int,
	...
	go func() {
		...
		for i := range chunks {
			...
			case chunkChan <- i:
			case <-done:
				// Stop queueing further chunks on interrupt (a plain `break`
				// here only exited the select, so all chunks kept queueing).
				return
			}
		}
	}()
	...
	for result := range resultChan {
		...
		if result.Err != nil && !*bestEffort {
			close(progressDone)
			return fmt.Errorf("chunk %d failed: %w", result.ChunkIdx, result.Err)
		}
	}
...
func mergeChunkFiles(results []ChunkResult, outPath string, appendMode bool) error {
	...
	out, err := os.OpenFile(filepath.Clean(outPath), flags, 0644) //nolint:gosec // G302/G304: operator-chosen output path; 0644 is intentional for shareable output
	if err != nil {
		return err
	}
	// Best-effort close for early-return paths; the success path below
...
func (el *ErrorLogger) Log(blockNum uint64, errType, errMsg string) {
	...
	el.mu.Lock()
	defer el.mu.Unlock()
	// Best-effort: a failed write to the error log must not abort the scan.
