...
// smoketestParams holds real URL parameter values (addresses, ids, rounds,
// token ids, timestamps) fetched from the database so the API queries are
// exercised with live values. Missing values fall back to type-valid defaults.
...
func init() { register(newSmoketestCmd()) }

func runSmoketest() error {
	base := smoketestAPIBase()
	db, err := connectSmoketestDB()
	if err != nil {
		return err
	}
	defer db.Close()

	p := fetchSmoketestParams(db)
	...
	endpoints := buildSmoketestEndpoints(p)
	...
	for _, ep := range endpoints {
		full := base + ep
		status, bad, reason := smoketestGet(client, full)
		if bad {
	...
	if len(failures) > 0 {
		...
		return fmt.Errorf("%d endpoint(s) failed", len(failures))
	}
...
func connectSmoketestDB() (*sql.DB, error) {
...
func fetchSmoketestParams(db *sql.DB) smoketestParams {
	p := smoketestParams{}
...
func buildSmoketestEndpoints(p smoketestParams) []string {
	const (
		off       = "0"
		lim       = "1000000"
		sortParam = "0"
	...
	add("/api/cosmicgame/bid/list/by_round/" + r + "/" + sortParam + "/" + off + "/" + lim)
...
// smoketestGet returns (statusCode, bad, reason). bad is true on transport
// error, non-200 status, or a JSON body that carries a non-empty "error" or
// "status":0.
func smoketestGet(client *http.Client, fullURL string) (int, bool, string) {
