...
Measured 2026-07-13 (HTTP-edge sprint; §6.2 infrastructure complete — §8.5
code documentation and the remaining §6.2.1/§6.3 endpoint work are the open
sections). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **141,858** (+2,282 this sprint: the compression/conditional middleware with their test matrices and benchmarks, `internal/version`, and the edge integration suites; generated `internal/api/v2/api.gen.go` is separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **294** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **47** (+`FuzzAcceptEncodingGzip` and `FuzzIfNoneMatch`: content negotiation and conditional-request evaluation never panic, stay deterministic, and the middleware only ever answers the full 200 representation or an empty 304 carrying the current validator) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **91.39%** (floor 91.3%; original denominator retained for historical continuity) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **94.25%** (floor 94.1%; generated API + test-only harnesses excluded by ADR-0006) | **≥90% — met**; commit gate active, next target 95% | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **93.56%** (floor 93.4%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **100.00%** (HTTP-edge sprint; the `--version` entrypoint tests execute every touched `main()` block) | **≥95%** | `make coverage-check` |
| Queries on sqlc | 0 | 0 — scaffolding retired with Phase 1 (D7 amended: hand-written pgx everywhere) | n/a | n/a |
| Routes on stdlib router | 0 | **all** — frozen v1 (188 OpenAPI operations incl. `/version`) plus 34 generated v2 operations (round resources, exact statistics/claims, participant/bidding analytics, five contract/live resources and the user profile/bid foundation), health, host-dispatched metadata and env-gated static assets on `net/http` via `internal/api/httpx`; **gin is out of the build graph** | all (v1 compat + v2) — active | route-drift tests + `go list -deps ./cmd/... ./internal/... \| rg -c gin-gonic` |
