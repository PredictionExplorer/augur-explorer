...
Measured 2026-07-12 (command-seam sprint slice 1). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **106,579** (+4,213 this sprint: the rwbot/ethtx/freezer-scan seam packages and their suites, net of the deleted duplicated bot and tx plumbing); generated `internal/api/v2/api.gen.go` is separate | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| golangci-lint issues | 433 (first run) | **85** capped / **346** uncapped (touched packages are clean; new diff is zero) | **0** | `golangci-lint run` |
| Test files | 17 | **205** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **88.85%** (floor 88.6%; original denominator retained for historical continuity) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **92.08%** (floor 91.8%; generated API + test-only harnesses excluded by ADR-0006) | **≥90% — met**; commit gate active, next target 95% | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **66.31%** (floor 66.0%; notibot/rwctl/freezer-scan covered by the command-seam slice 1; cgctl/opsctl/srvmonitor remain) | ratchet toward **≥90%** | `make coverage-check` |
...
- [~] Command-seam sprint: extract the operational logic of the zero-coverage
  ...
  - [ ] Slice 2 — `cmd/cgctl` + `cmd/opsctl` (merge `cgctl/internal/ethtx`
        into `internal/ethtx`; extract autobid, admin workflows, archive/db
        verify, smoketest)
