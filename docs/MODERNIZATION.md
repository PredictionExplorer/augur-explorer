...
Measured 2026-07-09 (ContractState extraction sprint). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | 67,875 (+1,140: the `contractstate` component and its test suite) | n/a (informational) | `rg --files internal cmd -g '*.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
| snake_case functions | ~700 | **259** (api 127, cg-etl 88 `proc_*` renamed with the Phase-3 port, notibot 24, rw-etl 10, rwalk-alarm 4, misc 6; **the entire store layer is 0**) | **0** | `rg "^func (\([^)]+\) )?[A-Za-z]+_[A-Za-z0-9_]*\(" --type go -c internal cmd` |
| `os.Exit` in library code (`internal/`) | ~560 | **11 matches = 4 real calls** (3 test-harness `TestMain`s + `primitives.Fatalf`; the rest are doc comments). The `cosmic_game_init` startup fatal is gone — `cosmicgame.Init` returns an error and `cmd/apiserver` decides. | **0** (allowed only in `cmd/*/main.go` startup) | `rg -c "os\.Exit" internal` |
| Dot-import files | ~70 | **15** | **0** | `rg -l '^\s*\. "github' --type go` |
| Package-level mutable globals (api + etl) | ~120 | ~33 (cg-etl ~25, rw-etl ~8; **the API contract-state globals are gone** — an injected `contractstate.State` replaces them, leaving only set-once infra handles) | ~0 (DI everywhere) | manual review per package |
| golangci-lint issues | 433 (first run) | **130** capped output (uncapped 474, down from 492; new packages lint-clean) | **0** | `golangci-lint run` |
| Test files | 17 | **96** | 100+ | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **28** | **25+** (see §4.4) — met; grows with Phase 3 | `rg "func Fuzz" internal cmd contracts -c` |
| Benchmarks | 0 | **4** (8 sub-benchmarks; baselines in `docs/benchmarks.md`, re-checked this sprint: B/op and allocs identical) | keep green vs baselines | `rg "func Benchmark" cmd internal -c` |
...
| Coverage on `internal/` (integration, enforced) | n/a | **69.1%** (ratchet floor raised 66% → 68% in CI) | **≥70%**, floor only moves up | `go test -race -tags=integration -coverprofile=c.out -coverpkg=./internal/... ./... && go tool cover -func=c.out \| tail -1` |
...
- [ ] Remove gin from go.mod once v1 compat runs on stdlib
