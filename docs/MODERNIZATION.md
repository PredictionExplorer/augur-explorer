...
Measured 2026-07-09 (stdlib router sprint). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | 69,474 (+1,599: `httpx` + `routes` + router/middleware/static tests) | n/a (informational) | `rg --files internal cmd -g '*.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| golangci-lint issues | 433 (first run) | **127** capped output (uncapped 471, down from 474; new packages lint-clean) | **0** | `golangci-lint run` |
| Test files | 17 | **101** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **28** | **25+** (see §4.4) — met; grows with Phase 3 | `rg "func Fuzz" internal cmd contracts -c` |
| Benchmarks | 0 | **4** (8 sub-benchmarks; baselines in `docs/benchmarks.md`; rate limiter re-based on the stdlib router this sprint: 19–24% faster, same allocs) | keep green vs baselines | `rg "func Benchmark" cmd internal -c` |
...
| Coverage on `internal/` (integration, enforced) | n/a | **69.5%** (floor 68%) | **≥70%**, floor only moves up | `go test -race -tags=integration -coverprofile=c.out -coverpkg=./internal/... ./... && go tool cover -func=c.out \| tail -1` |
