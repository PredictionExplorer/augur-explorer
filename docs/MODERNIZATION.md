...
Measured 2026-07-18 (transactional-ingestion sprint: the last open §7
engineering item is closed — every block's rows, handler writes and
processing watermark now commit in one pgx transaction through the
context-scoped `store.InTx` querier seam and a transaction-aware address
cache (D15, [ADR-0010](adr/0010-transactional-ingestion.md)); readers can
no longer observe partially applied blocks and replay idempotence is
defense in depth rather than load-bearing. Every remaining unchecked item
is external (frontend migration), production-gated (v1 removal,
run-loop.sh deletion, prod-RLP replay) or a decision (D1).) Update after
each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **186,756** (+1,024 this sprint: the store transaction seam, the atomicity fault-injection matrix, the InTx integration suite and the ingest benchmark; generated `internal/api/v2/api.gen.go` and `contracts/*/bindings.gen.go` are separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **357** (+3: the store InTx integration suite, the indexer atomicity matrix and the ingest benchmark) | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **92.25%** (29,815/32,321; floor 92.1% → 92.2%) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **95.29%** (23,788/24,964; floor 95.1% → 95.2%; generated API + test-only harnesses excluded by ADR-0006) | **≥95% — met**; commit gate active | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **94.61%** (27,239/28,791; floor 94.5%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **99.75%** (794/796; the two uncovered statements are the status-seed re-read defense arms, reachable only if a row inserted one statement earlier vanishes before its read-back) | **≥95%** | `make coverage-check` |
...
| Store queries on pgx-native pool | 0 | all (one shared `pgxpool` per process; every query resolves pool-vs-transaction per call through `store.Querier`, so ETL block transactions span the whole store — D15/ADR-0010) | all | manual |
