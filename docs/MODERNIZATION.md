...
Measured 2026-07-16 (docs-and-deprecation sprint; **Phase 4 is complete** —
§8.5 landed with a permanent revive doc-comment gate, testable examples and
reproducible pinned-abigen bindings — and v1 is formally deprecated: 180
spec `deprecated` flags plus RFC 9745/8594 response headers, drift-tested
against the runtime policy. Every remaining unchecked item is external
...
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **182,579** (+1,433 this sprint: the deprecation middleware/policy + tests, the `contracts/internal/buildjson` reproducibility tooling, four `Example*` files, the config `time.Time` kind and ~190 wire-model doc comments; generated `internal/api/v2/api.gen.go` and `contracts/*/bindings.gen.go` are separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| golangci-lint issues | 433 (first run) | **0** — **target met**, with a stricter set than the baseline (`ST1003` re-enabled, `godot` + `noctx` added, `gci` + `gofumpt` enforced as formatters, and since the docs sprint `revive` `exported`/`package-comments` — every exported symbol and package documented, permanently) | **0** — met | `golangci-lint run` |
| Test files | 17 | **349** (incl. 4 `Example*` files and the contracts artifact drift suite) | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **92.19%** (floor 92.1%; −0.01pp noise from the docs-sprint additions) | ratchet only; never lower | `make coverage-check` |
...
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **94.49%** (floor 94.4%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **100.00%** (docs-and-deprecation sprint: all 112 changed executable statements — deprecation middleware/policy, config time.Time kind, covergate diff scoping — covered) | **≥95%** | `make coverage-check` |
