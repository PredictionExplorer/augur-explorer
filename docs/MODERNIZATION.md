...
Measured 2026-07-10 (statistics-and-claims API-v2 sprint). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **84,265** (+3,672 this sprint: five statistics/claims APIs, exact-value keyset store pages and extensive tests); generated `internal/api/v2/api.gen.go` is a separate 3,521 lines | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **140** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **41** (including all eleven bounded opaque-cursor decoders) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
| Benchmarks | 0 | **4** (7 sub-benchmarks; baselines in `docs/benchmarks.md`) | keep green vs baselines | `rg "func Benchmark" cmd internal -c` |
| Coverage on `internal/` (unit) | 2.4% | **27.7%** | superseded by the integration ratchet below | `go test -coverprofile=c.out ./internal/... && go tool cover -func=c.out \| tail -1` |
| Coverage on `internal/` (integration, enforced) | n/a | **74.8%** (floor 74.7%) | **≥70%, floor only moves up — met** | `go test -race -tags=integration -coverprofile=c.out -coverpkg=./internal/... ./... && go tool cover -func=c.out \| tail -1` |
| Queries on sqlc | 0 | 0 — scaffolding retired with Phase 1 (D7 amended: hand-written pgx everywhere) | n/a | n/a |
| Routes on stdlib router | 0 | **all** — frozen v1 (187 OpenAPI operations) plus 16 generated v2 operations (round resources plus global statistics, counters, ROI and claims), health, host-dispatched metadata and env-gated static assets on `net/http` via `internal/api/httpx`; **gin is out of the build graph** | all (v1 compat + v2) — active | route-drift tests + `go list -deps ./cmd/... ./internal/... \| rg -c gin-gonic` |
| `context.Context` on store methods | 0% | **100% — 380 Repo methods (CosmicGame 318 + RandomWalk 62) + 22 base `Store` methods; `SQLStorage` and both wrappers are deleted** | 100% | `rg -c "func \(r \*Repo\)" internal/store/cosmicgame internal/store/randomwalk` |
...
- [x] httptest suite for v2 (same fixtures as §4.1, new goldens)
      *(2026-07-10 — 127 deterministic v2 goldens cover current/completed
      rounds, round bids, prizes, raffles, donations, global statistics,
      counters, all ROI sorts and bounded claim summaries/details: live
      success, unavailable-cache 503, keyset pages, lean items,
      bind/limit/cursor/sort/pool errors, open-round donation pages,
...
- [x] OpenAPI contract validation in tests (kin-openapi response validator)
      *(2026-07-10 — the embedded spec is validated, spec and generated router
      are compared bidirectionally (16 operations), and every v2 golden
...
- [~] Statistics/dashboard resources: exact DB-backed
      ...
      exact ETH wei/ERC-20 base units. Participant directories, bidding
      analytics, contract configuration and final dashboard migration remain.
- [ ] User resources
