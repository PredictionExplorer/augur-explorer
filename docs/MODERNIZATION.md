...
Measured 2026-07-12 (coverage-quality sprint 3). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **102,366** (+2,446 this sprint: notification/freezer/store/indexer/API failure-path suites, net of the deleted SequentialReader and the OAuth library trim); generated `internal/api/v2/api.gen.go` is separate | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
| snake_case functions | ~700 | **159** (api 127, notibot 24, rwalk-alarm 4, primitives 3, apiserver 1; **the store layer, both ETLs and the notify adapters are 0**) | **0** | `rg "^func (\([^)]+\) )?[A-Za-z]+_[A-Za-z0-9_]*\(" --type go -c internal cmd` |
...
| golangci-lint issues | 433 (first run) | **114** capped / **393** uncapped (touched packages are clean; new diff is zero) | **0** | `golangci-lint run` |
| Test files | 17 | **187** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **88.48%** (floor 88.2%; original denominator retained for historical continuity) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **91.82%** (floor 91.5%; generated API + test-only harnesses excluded by ADR-0006) | **≥90% — met**; commit gate active, next target 95% | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **59.65%** (floor 59.4%; exposes previously unmeasured command logic) | ratchet toward **≥90%** | `make coverage-check` |
