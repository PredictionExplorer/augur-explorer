...
Measured 2026-07-13 (config-and-slog sprint; Phase 4 complete). Update after
each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **139,576** (+2,177 this sprint: the typed config package with its loader/redaction tests, `DATABASE_URL` plumbing and the slog conversion tests; the dual-file log plumbing and env-reading helpers were deleted; generated `internal/api/v2/api.gen.go` is separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **289** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **45** (+`FuzzLoadValue`: the config loader never panics and never leaks a secret through the redacted attrs) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **91.32%** (floor 91.2%; original denominator retained for historical continuity) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **94.21%** (floor 94.1%; generated API + test-only harnesses excluded by ADR-0006) | **≥90% — met**; commit gate active, next target 95% | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **93.51%** (floor 93.3%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **95.82%** (config-and-slog sprint; the uncovered remainder is `main()` bodies) | **≥95%** | `make coverage-check` |
...
**Complete as of 2026-07-13 (config-and-slog sprint) — Phase 4 is done.**
