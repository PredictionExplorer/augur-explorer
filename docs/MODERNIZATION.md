...
Measured 2026-07-15 (user-activity sprint; the §6.2.1 user group is fully
done and `user/balances` is retired from v2 scope (D10) — §8.5 code
documentation, the §6.2.1 RandomWalk and global-directory slices and §6.3
are the open sections). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **156,469** (+3,712 this sprint: the activity store pages and one-query summaries with their integration suites, the v2 activity handlers/mappers/cursors with unit and fuzz coverage and the golden additions; generated `internal/api/v2/api.gen.go` is separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **320** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **55** (+`FuzzDecodeUserOwnedTokenCursor`: the owned-token cursor decoder only ever accepts canonical wallet-scoped payloads, and payloads of sibling cursor types fail structurally) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **91.92%** (floor 91.8% → 91.9%; spec-valid enum assertions on the mapped prize/mint/transfer taxonomies recovered the generated-glue dilution the six new operations added to this raw metric) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **94.63%** (floor 94.4% → 94.5%; generated API + test-only harnesses excluded by ADR-0006) | **≥90% — met**; commit gate active, next target 95% | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **93.93%** (floor 93.7% → 93.8%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **99.22%** (user-activity sprint; the three uncovered statements are the logically unreachable cursor-encode failure arm and one scan-error arm retained as defense in depth) | **≥95%** | `make coverage-check` |
| Queries on sqlc | 0 | 0 — scaffolding retired with Phase 1 (D7 amended: hand-written pgx everywhere) | n/a | n/a |
| Routes on stdlib router | 0 | **all** — frozen v1 (188 OpenAPI operations incl. `/version`) plus 56 generated v2 operations (round resources, exact statistics/claims, participant/bidding analytics, five contract/live resources, the user profile/bid foundation, the eight user winnings/donations histories, the eight user staking resources and the six user activity resources), health, host-dispatched metadata and env-gated static assets on `net/http` via `internal/api/httpx`; **gin is out of the build graph** | all (v1 compat + v2) — active | route-drift tests + `go list -deps ./cmd/... ./internal/... \| rg -c gin-gonic` |
...
- [ ] Global directories: CST token list/info/names/distribution/transfers,
      CosmicToken statistics/holders/supply histories, global marketing and
      staking statistics (the remaining v1-only groups after the user
      surface; tracked here so the v1 sunset inventory stays complete)
...
- [~] Publish v2 spec + changelog mapping every v1 path to its v2 replacement
      *(dashboard, user profile/bid, user winnings/donations, user staking
      and user activity mappings published in
      [api-v2-migration.md](api-v2-migration.md) — the user-scoped surface
      is fully mapped and `user/balances` is documented as retired from v2
      scope (D10); the global token/marketing/staking directories and
      RandomWalk groups remain)*
