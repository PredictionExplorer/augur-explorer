...
Measured 2026-07-13 (Phase-4 idiomatic-sweep sprint). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **137,399** (+67 net this sprint: dead Augur primitives and unreachable error arms deleted, new module constructors and coverage tests added; generated `internal/api/v2/api.gen.go` is separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **284** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **44** (three targets retired with their dead primitives helpers; all cursor/decoder/invariant targets remain) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **91.24%** (floor 91.0%; original denominator retained for historical continuity) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **94.14%** (floor 93.9%; generated API + test-only harnesses excluded by ADR-0006) | **≥90% — met**; commit gate active, next target 95% | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **93.15%** (floor 92.9%; `cmd/apiserver` now runs behind a tested `run(ctx, getenv)` seam like the ETL mains) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **96.70–98.05%** across the three idiomatic-sweep commits | **≥95%** | `make coverage-check` |
