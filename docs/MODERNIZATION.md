...
Measured 2026-07-10 (bidding-analytics API-v2 sprint). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **88,965** (+2,051 this sprint: five bounded analytics resources, optimized store projections and extensive tests); generated `internal/api/v2/api.gen.go` is a separate 5,466 lines | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **147** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **44** (including all twelve bounded opaque-cursor decoders plus bidding-window and spike-detection invariants) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
...
| Coverage on `internal/` (integration, enforced) | n/a | **75.2%** (floor 75.1%) | **≥70%, floor only moves up — met** | `go test -race -tags=integration -coverprofile=c.out -coverpkg=./internal/... ./... && go tool cover -func=c.out \| tail -1` |
| Queries on sqlc | 0 | 0 — scaffolding retired with Phase 1 (D7 amended: hand-written pgx everywhere) | n/a | n/a |
| Routes on stdlib router | 0 | **all** — frozen v1 (187 OpenAPI operations) plus 27 generated v2 operations (round resources, exact statistics/claims, six participant directories and five bounded bidding analytics resources), health, host-dispatched metadata and env-gated static assets on `net/http` via `internal/api/httpx`; **gin is out of the build graph** | all (v1 compat + v2) — active | route-drift tests + `go list -deps ./cmd/... ./internal/... \| rg -c gin-gonic` |
| `context.Context` on store methods | 0% | **100% — 395 Repo methods (CosmicGame 333 + RandomWalk 62) + 22 base `Store` methods; `SQLStorage` and both wrappers are deleted** | 100% | `rg -c "func \(r \*Repo\)" internal/store/cosmicgame internal/store/randomwalk` |
...
- [x] httptest suite for v2 (same fixtures as §4.1, new goldens)
      *(2026-07-10 — 180 deterministic v2 goldens cover current/completed
      ...
      participant directories plus five bidding analytics resources: live
      success, unavailable-cache 503, keyset pages, lean items, bounded time
      series, decimal-string percentages,
...
- [x] OpenAPI contract validation in tests (kin-openapi response validator)
      *(2026-07-10 — the embedded spec is validated, spec and generated router
      are compared bidirectionally (27 operations), and every v2 golden
...
- [~] Statistics/dashboard resources: exact DB-backed
      ...
      reads carry a five-second deadline.
      Contract configuration and final dashboard migration remain.
- [ ] User resources
