...
Measured 2026-07-15 (user-histories sprint; the §6.2.1 user winnings and
donations groups are done — §8.5 code documentation, the remaining §6.2.1
user slices (staking/transfers/tokens/marketing/balances), RandomWalk v2 and
§6.3 are the open sections). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **147,395** (+5,537 this sprint: eight user-history store pages with their integration suites, the v2 handlers/mappers/cursors with unit and fuzz coverage, and the golden additions; generated `internal/api/v2/api.gen.go` is separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **310** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **50** (+`FuzzDecodeUserPrizeCursor`, `FuzzDecodeUserEventCursor` and `FuzzDecodeUserDonatedErc20Cursor`: the three user-history cursor decoders only ever accept canonical wallet-and-resource-scoped payloads) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **91.35%** (floor 91.3%; this raw metric includes generated code, so regenerating `api.gen.go` for eight new operations costs ~0.04pp even at 98% changed-code coverage) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **94.40%** (floor 94.1% → 94.3%; generated API + test-only harnesses excluded by ADR-0006) | **≥90% — met**; commit gate active, next target 95% | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **93.70%** (floor 93.4% → 93.6%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **98.20%** (user-histories sprint; the ten uncovered statements are the logically unreachable cursor-encode failure arms retained as defense in depth) | **≥95%** | `make coverage-check` |
| Queries on sqlc | 0 | 0 — scaffolding retired with Phase 1 (D7 amended: hand-written pgx everywhere) | n/a | n/a |
| Routes on stdlib router | 0 | **all** — frozen v1 (188 OpenAPI operations incl. `/version`) plus 42 generated v2 operations (round resources, exact statistics/claims, participant/bidding analytics, five contract/live resources, the user profile/bid foundation and the eight user winnings/donations histories), health, host-dispatched metadata and env-gated static assets on `net/http` via `internal/api/httpx`; **gin is out of the build graph** | all (v1 compat + v2) — active | route-drift tests + `go list -deps ./cmd/... ./internal/... \| rg -c gin-gonic` |
...
- [~] User resources *(2026-07-11 — foundation landed:
      ...
      the fourteen user-scoped read indexes. Remaining
      staking/transfer/token/marketing histories and live balances stay
...
- [~] Publish v2 spec + changelog mapping every v1 path to its v2 replacement
      *(dashboard, user profile/bid and user winnings/donations mappings
      published in [api-v2-migration.md](api-v2-migration.md); remaining
      user staking/transfer/token histories, RandomWalk, CosmicToken and
      marketing groups remain)*
