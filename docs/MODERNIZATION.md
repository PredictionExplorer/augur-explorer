...
Measured 2026-07-12 (coverage-quality sprint 2). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **99,920** (+3,553 this sprint: API boundary/invariant tests, deterministic dependency seams and reliability hardening, net of dead compatibility branches removed); generated `internal/api/v2/api.gen.go` is separate | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **175** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **83.73%** (floor 83.5%; original denominator retained for historical continuity) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **86.07%** (floor 85.8%; generated API + test-only harnesses excluded by ADR-0006) | **≥90%**, then hold | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **56.26%** (floor 56.0%; exposes previously unmeasured command logic) | ratchet toward **≥90%** | `make coverage-check` |
...
- [ ] Coverage-quality sprint 3: raise handwritten internal from 86.07% to at
      ...
      The 88→90 sprint extracts operational command logic behind testable seams
      before activating the permanent commit gate.
