...
Measured 2026-07-18 (one-driver sprint: the ops toolchain — `internal/ops/*`,
`internal/toolutil` and the `opsctl` commands, the last `database/sql` +
`lib/pq` code paths — moved onto pgx-native queries behind narrow per-package
`Querier` seams; `github.com/lib/pq` left `go.mod` and a `depguard` rule keeps
it out permanently, so pgx is the repository's only PostgreSQL driver (D16).
Every remaining unchecked item is external (frontend migration),
production-gated (v1 removal, run-loop.sh deletion, prod-RLP replay) or a
decision (D1).) Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **186,961** (+205 this sprint: the shared scripted pgx test querier and the writer-level tests slightly outweigh the deleted `database/sql` plumbing, prepared-statement lifecycle and three scripted driver fakes; generated `internal/api/v2/api.gen.go` and `contracts/*/bindings.gen.go` are separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Benchmarks | 0 | **9** (41 leaf benchmarks incl. the ADR-0010 ingest-transaction pair; baselines in `docs/benchmarks.md`) | keep green vs baselines | `rg "func Benchmark" cmd internal -c` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **92.26%** (29,867/32,374; floor 92.2%) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **95.28%** (23,719/24,894; floor 95.2%; generated API + test-only harnesses excluded by ADR-0006) | **≥95% — met**; commit gate active | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **94.61%** (27,161/28,709; floor 94.5%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **99.69%** (321/322; the one uncovered statement is the pool-construction failure arm behind an already-validated config) | **≥95%** | `make coverage-check` |
