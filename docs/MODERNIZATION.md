...
Measured 2026-07-10 (round-raffles API-v2 sprint). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **78,547** (+1,923 this sprint: two round-raffle APIs, exact-wei/keyset store pages and extensive tests); generated `internal/api/v2/api.gen.go` is a separate 1,957 lines | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **132** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **35** (including all five bounded opaque-cursor decoders) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
...
| Coverage on `internal/` (integration, enforced) | n/a | **74.1%** (floor 74%) | **≥70%, floor only moves up — met** | `go test -race -tags=integration -coverprofile=c.out -coverpkg=./internal/... ./... && go tool cover -func=c.out \| tail -1` |
| Queries on sqlc | 0 | 0 — scaffolding retired with Phase 1 (D7 amended: hand-written pgx everywhere) | n/a | n/a |
| Routes on stdlib router | 0 | **all** — frozen v1 (187 OpenAPI operations) plus 8 generated v2 operations (current/completed rounds, round bids, prizes + raffles), health, host-dispatched metadata and env-gated static assets on `net/http` via `internal/api/httpx`; **gin is out of the build graph** | all (v1 compat + v2) — active | route-drift tests + `go list -deps ./cmd/... ./internal/... \| rg -c gin-gonic` |
| `context.Context` on store methods | 0% | **100% — 370 Repo methods (CosmicGame 308 + RandomWalk 62) + 22 base `Store` methods; `SQLStorage` and both wrappers are deleted** | 100% | `rg -c "func \(r \*Repo\)" internal/store/cosmicgame internal/store/randomwalk` |
...
- [x] httptest suite for v2 (same fixtures as §4.1, new goldens)
      *(2026-07-10 — 60 deterministic v2 goldens cover current/completed
      rounds, round bids, prizes and raffles: live success, unavailable-cache
      503, keyset pages, lean items, bind/limit/cursor/pool errors,
      open/missing 404s and cancelled-context 500s; v1's 196 parity + 12 error
...
- [x] OpenAPI contract validation in tests (kin-openapi response validator)
      *(2026-07-10 — the embedded spec is validated, spec and generated router
      are compared bidirectionally (8 operations), and every v2 golden
...
- [~] Round prize/raffle/donation sub-resources: cursor-paginated
