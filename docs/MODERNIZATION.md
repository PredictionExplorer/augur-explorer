...
Measured 2026-07-12 (command-seam sprint slice 3). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **137,332** (+6,892 this sprint: the srvmonitor engine + termbox seams, urlalarm/imggen/freezer-verify engines and the monitor/ETL-main test suites; generated `internal/api/v2/api.gen.go` is separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
| snake_case functions | ~700 | **131** (api 127, primitives 3, apiserver 1; **every binary, the store/indexer/ops layers and the notify stack are 0**) | **0** | `rg "^func (\([^)]+\) )?[A-Za-z]+_[A-Za-z0-9_]*\(" --type go -c internal cmd` |
...
| golangci-lint issues | 433 (first run) | **36** capped / **258** uncapped (all remaining issues are the legacy v1 api/apiserver packages; all touched packages are clean; new diff is zero) | **0** | `golangci-lint run` |
| Test files | 17 | **282** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **90.73%** (floor 90.5%; original denominator retained for historical continuity) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **93.53%** (floor 93.3%; generated API + test-only harnesses excluded by ADR-0006) | **≥90% — met**; commit gate active, next target 95% | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **92.03%** (floor 91.8%; **every binary is tested wiring** — the remaining uncovered code is `main()` bodies and unreachable error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **95.07%** for slice 3 | **≥95%** | `make coverage-check` |
