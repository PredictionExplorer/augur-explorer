...
Measured 2026-07-12 (command-seam sprint slice 2b). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **130,440** (+17,227 this sprint: the six extracted ops engines, shared log scanner and exhaustive command/SQL/RPC/filesystem coverage; generated `internal/api/v2/api.gen.go` is separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
| snake_case functions | ~700 | **135** (api 127, rwalk-alarm 4, primitives 3, apiserver 1; **the store/indexer/ops layers, both ETLs and notify stack are 0**) | **0** | `rg "^func (\([^)]+\) )?[A-Za-z]+_[A-Za-z0-9_]*\(" --type go -c internal cmd` |
...
| golangci-lint issues | 433 (first run) | **39** capped / **262** uncapped (all touched packages are clean; new diff is zero) | **0** | `golangci-lint run` |
| Test files | 17 | **251** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **90.19%** (floor 90.0%; original denominator retained for historical continuity) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **93.09%** (floor 93.0%; generated API + test-only harnesses excluded by ADR-0006) | **≥90% — met**; commit gate active, next target 95% | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **85.48%** (floor 85.2%; all command-seam binaries except `srvmonitor` are covered) | ratchet toward **≥90%** | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **97.80%** for slice 2b | **≥95%** | `make coverage-check` |
