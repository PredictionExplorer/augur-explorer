...
Measured 2026-07-11 (coverage-quality sprint 1). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **96,367** (+3,313 this sprint: tested coverage tooling, staged commit gates, high-risk branch tests and narrow reliability fixes); generated `internal/api/v2/api.gen.go` is separate | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| golangci-lint issues | 433 (first run) | **116** capped / **393** uncapped (coverage sprint cleaned touched tool utilities; new diff is zero) | **0** | `golangci-lint run` |
| Test files | 17 | **167** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **79.07%** (floor 78.8%; original denominator retained for historical continuity) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **80.43%** (floor 80.1%; generated API + test-only harnesses excluded by ADR-0006) | **≥90%**, then hold | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **52.80%** (floor 52.5%; exposes previously unmeasured command logic) | ratchet toward **≥90%** | `make coverage-check` |
...
- [x] World-class coverage policy and shared CI/commit gate implementation
      *(2026-07-11 — [ADR-0006](adr/0006-coverage-policy.md):
      ...
      remain deferred while coverage climbs toward 90%. The original
      denominator remains ratcheted at 78.8%; handwritten internal is 80.43%
      with an 80.1% floor, all production is truthfully tracked at 52.80%
      with a 52.5% floor, and changed executable Go requires 95%. Generated
      API and test-only harness code are excluded only from the new canonical
      metrics, never from the historical continuity metric. The authoritative
      race-enabled Sprint 1 profile closes at 79.07% legacy / 80.43%
      handwritten internal / 52.80% all production, with floors 78.8% /
      80.1% / 52.5%.)*
- [ ] Activate the fail-closed Git commit coverage gate **only after**
      ...
      succeeds, and never lower the floor afterward. Until then, commits are
      allowed and CI alone enforces the rising global ratchets plus 95% changed
      code coverage.
