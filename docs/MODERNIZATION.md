...
Measured 2026-07-16 (RandomWalk sprint; the v2 read inventory is complete —
every v1 read surface has a bounded v2 replacement. §8.5 code
documentation, the deferred §6.2.1 ranking mini-app slice and the external
§6.3 migration steps are the open sections). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **177,138** (+9,480 this sprint: the complete RandomWalk v2 read surface — store pages, handlers, cursors, mappers — plus seven layered test files, 69 HTTP goldens and benchmarks; generated `internal/api/v2/api.gen.go` is separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **337** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **68** (+5: the RandomWalk token-directory, per-token event, order-book, ledger and owned-token cursor decoders; cross-resource, cross-filter, cross-sort and cross-wallet payloads fail closed) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
| Benchmarks | 0 | **7** (39 sub-benchmarks; baselines in `docs/benchmarks.md`) | keep green vs baselines | `rg "func Benchmark" cmd internal -c` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **92.01%** (floor 92.0%; the 69 new HTTP goldens, the missing-required-parameter sweep across all 92 operations and the table-hiding failure-propagation tests hold the raw generated-code-inclusive metric above its ratchet after seventeen new operations) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **95.10%** (floor 94.8% → 95.0%; generated API + test-only harnesses excluded by ADR-0006) | **≥95% — met**; commit gate active | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **94.41%** (floor 94.1% → 94.3%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **98.49%** (RandomWalk sprint; the 22 uncovered statements are logically unreachable cursor-encode failure arms, the 64-bit `checkedInt` guard and defensive second-query/row-scan arms) | **≥95%** | `make coverage-check` |
| Queries on sqlc | 0 | 0 — scaffolding retired with Phase 1 (D7 amended: hand-written pgx everywhere) | n/a | n/a |
| Routes on stdlib router | 0 | **all** — frozen v1 (188 OpenAPI operations incl. `/version`) plus 92 generated v2 operations (round resources, exact statistics/claims, participant/bidding analytics, five contract/live resources, the complete user surface, global token/CosmicToken/marketing directories, nine global staking resources and the seventeen-operation RandomWalk group), health, host-dispatched metadata and env-gated static assets on `net/http` via `internal/api/httpx`; **gin is out of the build graph** | all (v1 compat + v2) — active | route-drift tests + `go list -deps ./cmd/... ./internal/... \| rg -c gin-gonic` |
| `context.Context` on store methods | 0% | **100% — 442 Repo methods (CosmicGame 380 + RandomWalk 62) + 23 base `Store` methods; `SQLStorage` and both wrappers are deleted** | 100% | `rg -c "func \(r \*Repo\)" internal/store/cosmicgame internal/store/randomwalk` |
...
- [ ] RandomWalk beauty-contest ranking slice (the interactive mini-app —
      `explore/random`, rating order, vote count, sign-challenge,
      beauty pairs and the two wallet-signed POST routes — needs the v2
      write conventions; deliberately deferred by D12)
