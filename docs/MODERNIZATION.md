...
Measured 2026-07-16 (ranking-write sprint; the §6.2.1 endpoint inventory is
complete — every v1 surface except the contract-pinned metadata route has a
bounded v2 replacement, and the first write conventions are decided
(D13/ADR-0008). §8.5 code documentation and the external §6.3 migration
steps are the open sections). Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **181,146** (+4,008 this sprint: the shared `internal/beautyrank` core, the v2 ranking store/handlers/cursors/middleware and four layered test files incl. the first v2 mutation-flow suite; generated `internal/api/v2/api.gen.go` is separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **340** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **92.20%** (floor 92.0% → 92.1%; the 28 mutation/read goldens and the coverage-directed follow-ups — semantic cursor rejection, admin outcome failures, duplicate-nonce conflicts, the hidden-nonce-table propagation test — lift the raw generated-code-inclusive metric across seven new operations) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **95.20%** (floor 95.0% → 95.1%; generated API + test-only harnesses excluded by ADR-0006) | **≥95% — met**; commit gate active | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **94.50%** (floor 94.3% → 94.4%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **99.51%** (ranking-write sprint; the 2 uncovered statements are the logically unreachable next-cursor encode arm — every row is validated before encoding) | **≥95%** | `make coverage-check` |
| Queries on sqlc | 0 | 0 — scaffolding retired with Phase 1 (D7 amended: hand-written pgx everywhere) | n/a | n/a |
| Routes on stdlib router | 0 | **all** — frozen v1 (188 OpenAPI operations incl. `/version`) plus 99 generated v2 operations (round resources, exact statistics/claims, participant/bidding analytics, five contract/live resources, the complete user surface, global token/CosmicToken/marketing directories, nine global staking resources, the seventeen-operation RandomWalk group and the seven-operation ranking slice with the first three POSTs), health, host-dispatched metadata and env-gated static assets on `net/http` via `internal/api/httpx`; **gin is out of the build graph** | all (v1 compat + v2) — active | route-drift tests + `go list -deps ./cmd/... ./internal/... \| rg -c gin-gonic` |
