...
Measured 2026-07-22 (edge-hardening sprint, §8.7/D21: every request body is
bounded at 1 MiB with per-family 413 rendering, the FAQ proxy caps buffered
upstream responses, public TLS floors at 1.2 with `http.Server` records on
slog, and the shared pgx pool exports thirteen `rwcg_db_pool_*` saturation/
churn series from the apiserver and both ETLs. Every remaining unchecked
item is external (frontend migration), production-gated (v1 removal,
...
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **192,367** (+1,277 this sprint: the body-cap middleware, pool collector and their unit/integration/benchmark matrices; generated `internal/api/v2/api.gen.go` and `contracts/*/bindings.gen.go` are separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **384** (+5: body-cap unit/integration matrices, 413-parity and spec-inventory tests, pool-collector exposition/workload suites) | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Benchmarks | 0 | **10** (44 leaf benchmarks incl. the request-body-cap middleware trio; all measured loops on Go 1.24 `b.Loop()` since the modern-idioms sprint, baselines in `docs/benchmarks.md`) | keep green vs baselines | `rg "func Benchmark" cmd internal -c` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **92.31%** (30,196/32,710; floor 92.3%) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **95.41%** (24,015/25,170; floor **95.4%**; generated API + test-only harnesses excluded by ADR-0006) | **≥95% — met**; commit gate active | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **95.01%** (27,609/29,060; floor **95.0%**) | **≥95% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **98.36%** (120/122) | **≥95%** | `make coverage-check` |
