...
production-exported RLP samples), a decision (D1), or finish-line closure
(the final dashboard/document-retirement boxes). Update after each phase.)

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **196,058** (+1,386 this sprint: typed v1/v2/operational probe engine, shared API policy, validation and unit/integration/fuzz matrices; generated `internal/api/v2/api.gen.go` and `contracts/*/bindings.gen.go` are separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **402** (+3: v2 smoke catalog/contract/fail-closed matrix, shared deprecation policy, and the full 97-route production-router integration proof) | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **73** (+2: v2 probe URL binding/escaping and shared deprecation-path determinism; full fleet retained) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **92.39%** (30,528/33,042; floor 92.3%) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **95.47%** (24,347/25,502; floor **95.4%**; generated API + test-only harnesses excluded by ADR-0006) | **≥95% — met**; commit gate active | `make coverage-check` |
