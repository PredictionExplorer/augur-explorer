...
Measured 2026-07-15 (global-directories sprint; the global token,
CosmicToken and marketing directories are on v2 and the two RPC-backed v1
routes are resolved by D11 — §8.5 code documentation, the §6.2.1
RandomWalk and global-staking slices and §6.3 are the open sections).
Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **162,912** (+6,443 this sprint: the global directory/holder/supply/marketing store pages and one-snapshot statistics with their integration suites, the v2 handlers/mappers/cursors with unit and fuzz coverage, the treasurer cache extension and the golden additions; generated `internal/api/v2/api.gen.go` is separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| golangci-lint issues | 433 (first run) | **0** — **target met**, with a stricter set than the baseline (`ST1003` re-enabled, `godot` + `noctx` added, `gci` + `gofumpt` enforced as formatters) | **0** — met | `golangci-lint run` |
| Test files | 17 | **326** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **59** (+4: the global-token cursor with its embedded filter scope, the token-scoped event cursor, and the supply-ledger and global-marketing cursors — every decoder only accepts canonical payloads of its own resource) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
| Benchmarks | 0 | **6** (30 sub-benchmarks; baselines in `docs/benchmarks.md`) | keep green vs baselines | `rg "func Benchmark" cmd internal -c` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **92.13%** (floor 91.9% → 92.0%; generated enum-domain and configuration-variant conformance tests recovered the dilution the ten new generated operations added to this raw metric) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **94.81%** (floor 94.5% → 94.7%; generated API + test-only harnesses excluded by ADR-0006) | **≥90% — met**; commit gate active, next target 95% | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **94.10%** (floor 93.8% → 94.0%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **98.64%** (global-directories sprint; the eleven uncovered statements are the four logically unreachable cursor-encode failure arms plus one scan-error and one jsonb-decode arm retained as defense in depth) | **≥95%** | `make coverage-check` |
| Queries on sqlc | 0 | 0 — scaffolding retired with Phase 1 (D7 amended: hand-written pgx everywhere) | n/a | n/a |
| Routes on stdlib router | 0 | **all** — frozen v1 (188 OpenAPI operations incl. `/version`) plus 66 generated v2 operations (round resources, exact statistics/claims, participant/bidding analytics, five contract/live resources, the user profile/bid foundation, the eight user winnings/donations histories, the eight user staking resources, the six user activity resources and the ten global token/CosmicToken/marketing directories), health, host-dispatched metadata and env-gated static assets on `net/http` via `internal/api/httpx`; **gin is out of the build graph** | all (v1 compat + v2) — active | route-drift tests + `go list -deps ./cmd/... ./internal/... \| rg -c gin-gonic` |
...
- [ ] Global staking statistics: staked-token membership, action
      ledgers/details, reward deposits (global and by round) and staker
      raffle mints (the remaining v1-only CosmicGame group; tracked here
      so the v1 sunset inventory stays complete)

### 6.3 Frontend migration

- [~] Publish v2 spec + changelog mapping every v1 path to its v2 replacement
      *(dashboard, user profile/bid, user winnings/donations, user staking,
      user activity and global token/CosmicToken/marketing mappings
      published in [api-v2-migration.md](api-v2-migration.md) —
      `user/balances` is documented as retired from v2 scope (D10) and the
      `ct/statistics` token metadata plus the marketing-wallet owner as
      retired with the request-time RPC (D11); the global staking
      statistics and RandomWalk groups remain)*
...
**Complete as of 2026-07-13 (config-and-slog sprint). §8.5 documentation is
Phase 4's last open section.**
