...
Measured 2026-07-23 (bounded-time sprint, §8.8/D22: every API request runs
under a 30s deadline with per-family 503 timeout rendering, PostgreSQL
enforces server-side statement and idle-in-transaction bounds behind the
context deadlines, the ETL engine wraps its RPC client so a black-holed
endpoint trips the breaker instead of hanging the process forever — even
mid-transaction on the shutdown-immune context — and every outbound call in
the notifiers, bots, probes and chain tools carries a deadline. Every
remaining unchecked item is external (frontend migration), production-gated
(v1 removal, run-loop.sh deletion, production-exported RLP samples) or a
decision (D1).) Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **194,672** (+2,305 this sprint: the deadline middleware, bounded clients/callers and their unit/integration/synctest/benchmark matrices; generated `internal/api/v2/api.gen.go` and `contracts/*/bindings.gen.go` are separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **399** (+15: deadline middleware/policy/counter suites, bounded-client synctest + hung-RPC rollback integration, ethcall, logscan fetch-timeout, store GUC unit + pg_sleep integration, Discord/WhatsApp/Twitter/autobid/srvmonitor/opsctl hang matrices) | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **92.35%** (30,312/32,823; floor 92.3%) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **95.44%** (24,131/25,283; floor **95.4%**; generated API + test-only harnesses excluded by ADR-0006) | **≥95% — met**; commit gate active | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **95.04%** (27,747/29,195; floor **95.0%**) | **≥95% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **98.23%** (222/226; only the two unreachable timeout-marshal fallbacks uncovered) | **≥95%** | `make coverage-check` |
