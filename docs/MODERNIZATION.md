...
Measured 2026-07-18 (v2-later-store-failure hardening sprint: 29 sequential
full-router cases now cover every distinct multi-stage GET-handler shape
whose later repository failure can be isolated safely. Every injected 500 is
OpenAPI-validated, pinned to the exact RFC 9457 internal problem and checked
for SQL/driver/credential leakage; the shared DDL seam quotes identifiers,
bounds rename/restore operations and resets cached pgx connection state. The
canonical v2 surface remains 102 operations and its mapping is complete.
...
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **185,732** (+378 this sprint: the 29-case full-router failure matrix and safer shared fault-injection plumbing; generated `internal/api/v2/api.gen.go` and `contracts/*/bindings.gen.go` are separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **354** (+1: v2 later-store-failure full-router matrix; incl. 4 `Example*` files and the contracts artifact drift suite) | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **92.18%** (29,735/32,256; floor 92.1%) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **95.22%** (23,708/24,899; floor 95.1%; generated API + test-only harnesses excluded by ADR-0006) | **≥95% — met**; commit gate active | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **94.55%** (27,159/28,726; floor 94.5%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **98.94%** (187/189; unchanged — this sprint modifies only ADR-0006-excluded test harnesses) | **≥95%** | `make coverage-check` |
...
| `context.Context` on store methods | 0% | **100% — 472 Repo methods (CosmicGame 384 + RandomWalk 88) + 23 base `Store` methods; `SQLStorage` and both wrappers are deleted** | 100% | `rg -c "func \(r \*Repo\)" internal/store/cosmicgame internal/store/randomwalk` |
