...
Measured 2026-07-23 (upstream-production-convergence sprint, §8.10/D24:
the 80-commit modernization stack is rebased onto upstream `a1eb87d6`,
CosmicSignatureGame V3 and AxiomZero marketplace behavior are ported into the
modern contract/store/indexer/API layers, and anomaly monitoring is
self-verifying. Bare `opsctl smoketest` validates all 101 safe v2 GETs; the
explicit v1 sweep covers 145 requests. Every remaining unchecked item is
external (frontend migration), production-gated (v1 removal, run-loop.sh
deletion, production-exported RLP samples and sentinel-row cleanup), a
decision (D1), or finish-line closure. Update after each phase.)

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **202,011** (+5,953 this sprint: V3/store/indexer/API, shared marketplace, monitoring protocols and their tests; generated `internal/api/v2/api.gen.go` and `contracts/*/bindings.gen.go` are separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **411** (+9: V3 ABI/store/indexer, collection-scoped marketplace, run-loop and anomaly freshness suites) | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **74** (+1 bounded anomaly-heartbeat parser; V3 joins the existing ABI/decode fuzz targets) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
| Benchmarks | 0 | **11** (46 leaf benchmarks incl. the request-deadline pair; all measured loops on Go 1.24 `b.Loop()` since the modern-idioms sprint, baselines in `docs/benchmarks.md`) | keep green vs baselines | `rg "func Benchmark" cmd internal -c` |
| Coverage on `internal/` (unit) | 2.4% | **66.5%** | superseded by the integration ratchet below | `go test -coverprofile=c.out ./internal/... && go tool cover -func=c.out \| tail -1` |
| Legacy `internal/` integration coverage (enforced) | n/a | **92.34%** (31,585/34,205; floor 92.3%) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **95.55%** (25,167/26,340; floor **95.4%**; generated API + test-only harnesses excluded by ADR-0006) | **≥95% — met**; commit gate active | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **95.14%** (28,817/30,288; floor **95.0%**) | **≥95% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **95.89%** (1,702/1,775 across the combined upstream-convergence ports) | **≥95%** | `make coverage-check` |
...
| `context.Context` on store methods | 0% | **100% — 497 domain Repo methods (CosmicGame 408 + RandomWalk 89), 9 shared marketplace methods + 23 base `Store` methods; `SQLStorage` and both wrappers are deleted** | 100% | `rg -c "func \(r \*Repo\)" internal/store/cosmicgame internal/store/randomwalk internal/store/marketplace` |
...
- [x] httptest suite for v2 (same fixtures as §4.1, new goldens)
      ...
      *(extended 2026-07-18 — 29 sequential full-router fault-injection cases
      drive each distinct isolatable multi-stage GET shape beyond its
      successful resolver/existence lookup, then hide the later domain table.
      Every response must be the exact opaque RFC 9457 internal problem, omit
      SQL/driver/credential details and validate against OpenAPI. The shared
      DDL seam uses quoted identifiers, bounded contexts, guaranteed restore
      and pgx pool resets; all existing goldens remain byte-identical.)*
...
- [x] RandomWalk beauty-contest ranking slice *(2026-07-16 — the ranking
      ...
      (D12).)*
- [x] CosmicGame bid moderation *(2026-07-17 — a completion audit found the
      three v1 `get_banned_bids`/`ban_bid`/`unban_bid` behaviors absent from
      the claimed-complete mapping. ADR-0009/D14 turns them into one
      addressable active-ban resource: public newest-first cursor-paginated
      `GET /api/v2/cosmicgame/moderation/banned-bids`, apiKey-secured
      `POST` with only `{bidId}` and a typed `201`, and
      `DELETE …/{bidId}` with bodyless `204`. The server derives the
      checksummed bidder from the canonical indexed bid; malformed/missing
      bids, duplicate bans and missing deletes receive specific opaque
      problems. Migration 00025 deterministically retains the newest
      historical duplicate then enforces one active ban per bid with a
      unique index. `AdminKey` (`X-Admin-Key` / `ADMIN_API_KEY`) joins the
      scopes-driven strict middleware beside `RankingAdminKey`; fixed-width
      SHA-256 values are compared in constant time and create/delete retain
      independent 2 rps / burst-5 buckets. Seventeen new twice-fetched/flow
      goldens bring the v2 suite to 546; store migration/page/write tests,
      full-router lifecycle/auth/rate-limit coverage, production boot wiring,
      generated response visitors and `FuzzDecodeBidBanCursor` cover the
      slice. All v1 parity goldens remain byte-identical. The v2 inventory is
      now genuinely complete at 102 operations.)*
...
- [x] Publish v2 spec + changelog mapping every v1 path to its v2 replacement
      *(complete 2026-07-16 — [api-v2-migration.md](api-v2-migration.md) maps
      ...
      half-mint-funds approximations (D12). The ranking mini-app (D13) and
      CosmicGame bid moderation (D14) now have full read/write mappings; the
      contract-pinned `metadata/{token_id}` route alone stays permanently
      outside v2 (D12), while FAQ remains an exempt upstream proxy.)*
...
- [x] v1 marked deprecated in spec; add `Deprecation`/`Sunset` headers
      *(2026-07-16 — all 180 v1 operations carry `deprecated: true` in
      ...
      operations); `policy.V1Deprecated` is the single policy shared by the
...
- [ ] Remove v1 layer + its goldens when traffic hits zero (final step, gated)
      *(measurable since 2026-07-16: `rwcg_http_requests_total` carries a
      `deprecated` label derived from the same `policy.V1Deprecated` policy
...
**Status: complete as of 2026-07-18 (transactional-ingestion sprint)** —
the engine core (loop, retry, metrics, slog) landed in the indexer-engine
sprint; the handler port finished the rewrite; the transactional-ingestion
sprint closed the last deferred item, so every §7 box is checked. Both ETL
binaries are pure wiring.
...
- [x] Status/progress persisted transactionally with the batch's inserts
      *(2026-07-18 — transactional-ingestion sprint, D15,
      ...
      events commits its block/transaction/evt_log rows, every handler's
      domain writes **and the processing watermark** in one `pgx`
      transaction. The querier abstraction the 2026-07-09 deferral said
      this needed exists now as `store.Querier` + context-scoped
      `Store.InTx` — all 472 repo methods and 23 base Store methods
      resolve pool-vs-transaction per call with zero signature changes —
      and the address LRU gained the tx-aware overlay (rollback discards,
      commit flushes; `LookupOrCreateAddress` moved to `ON CONFLICT DO
      NOTHING` because a lost insert race inside a transaction aborted it,
      SQLSTATE 25P02). Readers can no longer observe partially applied
      blocks; delete-then-insert replay idempotence is demoted from
...
- [x] Go 1.26 idiom sweep + permanent `go fix` gate: the stdlib modernize
      fixer suite (`interface{}`→`any`, `min`/`max`, `maps.Copy`,
      `slices.Contains`, `strings.SplitSeq`/`CutSuffix`, `reflect.TypeFor`,
      `sync.WaitGroup.Go`, Go 1.26 `new(expr)`, no-op struct `omitempty`
      removal) applied across 56 files under both build-tag sets, with the
      now-dead hand-rolled pointer helpers deleted; CI's lint job fails on
      any pending fix (`go fix -diff`, plain and `-tags=integration`) and
      `make fix` applies them locally. Legacy `sort.Slice`/`sort.Sort`
      call sites (~40) converted to `slices.SortFunc`/`SortStableFunc`
      with `cmp.Compare`/`cmp.Or` comparators — the `sort` package and its
      last `sort.Interface` implementation (`byKeyValue` in the OAuth1
      signer, pinned by its known-answer vector) are out of the
      non-generated tree, as is `math/rand` v1 (srvmonitor's image
      spot-check seam now rides `math/rand/v2.Int64N`)
      *(2026-07-18, modern-idioms sprint)*
- [x] Lint frontier v2: `gocritic`, `perfsprint`, `sloglint`, `recvcheck`,
      `embeddedstructfieldcheck`, `iface`, `mirror`, `exptostd`,
      `fatcontext` and `nilnesserr` enabled at zero findings (~120 fixed:
      constant `fmt.Errorf`→`errors.New`, Sprintf→`strconv`/concatenation,
      `slog.DiscardHandler`, unified covergate receivers, two aliasing
      `append` cross-slice concatenations → `slices.Concat`). gocritic's
      `exitAfterDefer` audit restructured all ten signal-handling binaries
      behind `runMain` seams so every deferred cleanup (signal release,
      log rotation, terminal restore) runs before `main` decides the exit
      code — each seam pinned by an error-propagation test
      *(2026-07-18, modern-idioms sprint)*
...
### 8.7 Bounded edges and pool observability

**Complete as of 2026-07-22 (edge-hardening sprint; D21). The process edge
accepts no unbounded client input and the shared DB pool is observable.**

- [x] Global request-body cap: `common.MaxRequestBody` (1 MiB, fixed — D21)
      sits in the shared chain between the metrics extras and
      `ConditionalETag`. Declared oversized bodies answer 413 without a
      byte read — RFC 9457 problem+json under `/api/v2/` (byte-identical
      to a handler-level problem; `common.ProblemTypeBase` is now the one
      source both layers build from), the legacy `{"status":0,"error":…}`
      envelope elsewhere. Undeclared overflows fail at first read via
      `http.MaxBytesReader`: the generated v2 decoders map
      `*http.MaxBytesError` to the same 413 problem in
      `writeRequestError`, the FAQ proxy answers the same legacy 413, and
      the frozen v1 handlers keep their `400 {"error":"http: request body
      too large"}` bind shape. The three body-accepting v2 operations
      declare `413` (`RequestTooLarge` response, spec v0.20.0,
      regenerated); an integration matrix walks every non-GET route on the
      production router and pins both shapes, plus a cap-minus-one vote
      body that must reach handler semantics.
- [x] FAQ proxy hygiene: buffered upstream responses cap at 4 MiB (larger
      answers a 502 without relaying; exactly-at-cap relays byte-identically)
      and the request-build 502 no longer echoes `err.Error()` — the
      underlying error is logged, the envelope is fixed text.
- [x] Transport floor: public TLS listeners set
      `MinVersion: tls.VersionTLS12` (integration-tested: a TLS 1.1-only
      handshake is refused, 1.2 succeeds on both listeners), and every
      `http.Server` — public, internal metrics, ETL metrics — routes its
      `ErrorLog` through the process slog logger at `WARN` instead of
      stderr, so handshake noise is structured and filterable.
- [x] DB-pool observability: `store.PoolCollector` exposes all thirteen
      `pgxpool.Stat` signals as `rwcg_db_pool_*` (saturation gauges,
      acquire/empty-acquire/cancel counters, acquire-wait seconds,
      connection churn). The apiserver registers it on the default
      registry and both ETLs on their injected registries — each paired
      with unregistration on shutdown (D20 ownership; pinned by tests that
      gather after run returns), with a loud startup error on registration
      conflict (covered in all three binaries). Exposition is pinned
      name-by-name with `testutil.CollectAndLint`/`CollectAndCompare`, a
      real-workload integration test proves the counters advance and the
      gauges stay consistent, and the apiserver boot test scrapes
      `/metrics` through the real internal listener.
      [operations.md](operations.md) documents the series and the
      saturation PromQL.
- [x] Benchmarks: `BenchmarkMaxRequestBody` (3 leaf cases) baselines the
      wrapper every request now traverses (~sub-µs full-router exchange;
      neighbours re-ran clean) — recorded in
      [benchmarks.md](benchmarks.md).

### 8.8 Bounded time (no unbounded waiting)

**Complete as of 2026-07-23 (bounded-time sprint; D22). §8.6 closed orphan
work, §8.7 closed unbounded input; this closes the third resource: no API
request, database statement or outbound call can wait forever. A black-holed
dependency now degrades to an explicit, observable, retried failure.**

- [x] API request deadline: `common.RequestDeadline` puts one 30s deadline
      (`common.DefaultRequestDeadline`, fixed — D22) on every request
      context from the shared chain (inside the metrics extras, like the
      body cap). Policy lives in `routes.RequestDeadlineFor`: everything is
      bounded except the FAQ proxy (its LLM upstream is bounded by the
      proxy's own 180s client + 4 MiB cap). A post-deadline internal error
      is intercepted at the header boundary and rendered as the family 503
      — RFC 9457 `request-timeout` problem under `/api/v2/` (built from
      `common.ProblemTypeBase`, byte-pinned), the legacy envelope elsewhere
      — while a handler that finishes just past the deadline still delivers
      its result and a client cancel keeps its old behavior. The middleware
      clones the request (`WithContext`), so it copies the mux-matched
      `Pattern` back for the outer access-log/metrics labels (pinned).
      `rwcg_http_request_timeouts_total{method,route}` counts every
      interception through `routes.Options.OnRequestTimeout`.
      [operations.md](operations.md) documents policy and alert.
- [x] Server-side DB bounds (defense in depth behind context deadlines):
      `store.Config.StatementTimeout` / `IdleInTxSessionTimeout` render as
      the PostgreSQL session GUCs on every pool connection. apiserver 30s
      (= the request deadline) / 5m; cg-etl, rw-etl, notibot and
      imggen-monitor 60s / 5m; operator CLIs deliberately keep the server
      defaults (heavy statements are legitimate; Ctrl-C cancels).
      Integration proof: `SHOW` round-trips, a runaway `pg_sleep` dies with
      SQLSTATE 57014 and the pool stays healthy, a client context deadline
      cancels a running statement promptly, and an `InTx` transaction
      stalled between statements is terminated and recovered from.
- [x] ETL chain RPC: `indexer.Config.RPCCallTimeout` (default
      `DefaultRPCCallTimeout`, 60s) wraps the `Client` seam in a
      `boundedClient` decorator at construction, so every present and
      future engine call — live loop, backfill, chain-split refetch, and
      the in-transaction lookups on the shutdown-immune `WithoutCancel`
      context — carries a per-call deadline ("finish the batch" can no
      longer mean "forever"). A hung call is an ordinary batch failure:
      backoff, adaptive batch shrinking and the breaker apply (synctest
      proofs), and a hung mid-transaction RPC rolls the whole block back
      with zero residue and converges on retry (integration proof against
      real PostgreSQL + testchain). Previously a black-holed endpoint hung
      the ETL forever without recording a single failure, and SIGTERM
      could not interrupt it.
- [x] Contract reads: new [internal/ethcall](../internal/ethcall) bounds
      `bind.ContractCaller`s (15s default). The CosmicGame handlers wrap
      their caller at construction (the donation-info and tokenURI reads
      run inside the per-block DB transaction), the startup
      contract-parameter sync runs over a read-bounded backend with
      ctx-carrying `CallOpts` (they were `context.Background()`), and the
      one v1 request-time RPC — live special winners — now shares the
      contractstate `rpcReadTimeout` with the refresh loops.
- [x] Outbound sweep, every unattended process: Twitter's OAuth fallback
      client is bounded at 2m (sized to chunked video uploads — never
      `http.DefaultClient` again), WhatsApp at 30s, the notibot Discord
      sink at 30s per REST call (rate-limit waits stay outside the bound),
      autobid's default dial injects a 30s-per-exchange HTTP client under
      its whole JSON-RPC surface, srvmonitor's RPC probe joins the other
      monitors at 10s, `logscan` bounds each fetch at 60s
      (`Options.FetchTimeout`; a timed-out fetch halves and retries) and
      the opsctl chain tools dial through a 90s-per-exchange client
      (above logscan's bound, so the scanner's deadline stays effective).
      `internal/ethtx` receipt waits were already bounded (2m).
- [x] `WriteTimeout` stays 0 on the public listeners, now with the
      truthful rationale: responses are fully buffered (nothing streams),
      the risk is large v1 arrays to slow clients, and handler execution is
      bounded by the request deadline instead — the write phase spends
      memory, not database or RPC capacity.
- [x] Benchmarks: `BenchmarkRequestDeadline` (2 leaf cases) baselines the
      per-request cost of the deadline wrapper (one timer + request clone,
      ~0.4µs over the exempt path) — recorded in
      [benchmarks.md](benchmarks.md).

### 8.9 V2 operational readiness and sunset-safe probes

**Complete as of 2026-07-23 (v2-operational-readiness sprint; D23). The
canonical API now has an out-of-process contract test, while routine probes
...
- [x] `opsctl smoketest` is a typed four-suite engine. Bare invocation runs
      ...
      response against its operation schema and rejects any `Deprecation` or
...
- [x] Compatibility and uptime are explicit. `--suite=v1` retains the explicit
      ...
      contract-cache GETs stay in the strict full-v2 suite (503 is a real
      user-visible failure) and out of uptime probes; `/readyz` remains the
      deliberate DB+draining signal rather than absorbing RPC cache state.
- [x] D6 policy is dependency-light and enforced at configuration time.
      `internal/api/policy` owns the one deprecated-prefix predicate used by
      router headers, request metrics, smoketest and srvmonitor without
      making operational packages import the full router. srvmonitor rejects
      malformed or deprecated internal/public web-probe URLs with aggregated,
      actionable startup errors; its lenient-internal/strict-public runtime
      behavior is unchanged. `.env.example` and both operator runbooks use
      `/readyz` internally and a stable v2 list publicly.
- [x] Compose gives the apiserver a `/readyz` healthcheck and an opt-in
      one-shot `smoke` profile that builds opsctl, waits for readiness and
      runs the no-credentials operational suite. `docker compose config`
      validates the topology. Runtime execution in this development
...
- [x] Verification: full race+shuffle unit and integration gates, lint (0),
      ...
      coverage gate pass. Coverage closes at 92.39% legacy internal,
      95.47% handwritten internal, 95.07% all production and 97.76% changed
      executable Go; existing floors remain valid.

### 8.10 Upstream production convergence

**Complete as of 2026-07-23 (upstream-production-convergence sprint; D24).
The modern rewrite now contains every production behavior from upstream
`master` at `a1eb87d6`, without resurrecting the deleted legacy layout.**

- [x] History: fetched the stale tracking ref to `a1eb87d6`, anchored the old
      tip, and rebased all 80 modernization commits. The only structural
      conflict was the first rewrite commit: its exact modern tree was
      retained while upstream `rwcg/`/`previous-code/` behavior became the
      port checklist. The pre/post modern trees were identical before the
      intentional ports; `origin/master` is now an ancestor. The remote
      feature branch remains untouched at `2b61953e` (local-only rebase).
- [x] CosmicSignatureGame V3: the committed build artifact and generated
      bindings include V3; migrations add multi-NFT main claims, normalized
      current/previous-bidder CST rewards, champion-duration snapshots, five
      admin event families and `cg_live_state_updates`. Both V2/V3 topics stay
      registered. Claim-time duration reads are bounded/non-fatal with startup
      recovery; evented contract sync is read-only drift reporting, never a
      synthetic event writer. V3 mechanics/configuration, exact bid splits,
      durations and multi-NFT fields are available through v1 compatibility
      and generated v2 contracts. Historical `0xcccc…` rows are preserved for
      operator-audited cleanup after rollout.
- [x] AxiomZero marketplace: upstream's three Cosmic Signature v1 routes are
      preserved byte-compatibly and deprecated normally. A shared
      collection-scoped pgx repository prevents cross-collection leakage and
      also powers four canonical v2 resources (offers, offer history, trades,
      floor price) with exact wei and resource/collection/sort-bound cursors.
      Concurrent collection indexes keep reads proportional to the requested
      collection. Existing RandomWalk goldens remain byte-identical; the
      catalogs are now 145 v1 requests and 101 safe v2 GET operations.
- [x] Monitoring: srvmonitor loads six event-table DB slots. `loganomaly`
      atomically emits a `#TS=` heartbeat even for zero findings; srvmonitor
      strips it, bounds parsing, displays freshness and emits stable repeated
      alarms after `ANOMALY_STALE_SECS` (30m default), while markerless files
      remain compatible. The transitional script resolves its sibling binary
      and fails when it disappears; a hardened persistent systemd timer owns
      the permanent five-minute one-shot model.
- [x] Quality proof: each logical port passed the managed race-enabled
      coverage commit hook. The combined tree passes deterministic generation,
      build, vet, lint (0), both `go fix` sets, full race+shuffle unit and
      integration suites, all 74 fuzz targets, migration up/down, systemd/
      shell behavior and govulncheck (0 reachable). Coverage is 92.34% legacy
      internal, 95.55% handwritten internal, 95.14% all production and 95.89%
      across the complete 1,775-statement convergence patch.
...
| E2E smoke | `opsctl smoketest` v2/operational suites + Compose smoke profile | 2, 4 | all safe v2 GETs validate through the production router; routine whole-stack probes stay D6-safe |
...
| D13 | v2 write conventions (unblocks the D12-deferred ranking slice) | ad-hoc per endpoint / REST-style POST+201+problems / RPC-style command envelope | **decided 2026-07-16: POST + 201 typed body + RFC 9457 problems** ([ADR-0008](adr/0008-api-v2-writes.md)) — record creation uses POST and answers 201 with the resulting state (no success envelopes, no Location for non-addressable ledger rows); body/validation failures are 400 problems with specific kinds, uniqueness conflicts 409, admin auth is a spec-declared apiKey header (same `X-Ranking-Admin-Key` / `RANKING_ADMIN_KEY`→`ADMIN_API_KEY` contract, fail-closed 503 / 401), wallet auth stays EIP-191 `personal_sign` over the byte-identical v1 message (shared `internal/beautyrank` builder), and per-operation rate limits run as generated strict middleware answering spec-declared 429 + `Retry-After` problems. The side-effecting `sign-challenge` GET became `POST ranking/challenges`. D14/ADR-0009 clarifies addressable resource removal as DELETE + 204. |
| D14 | v2 bid-moderation resource shape | mirror v1 POST commands / addressable active-ban resource | **decided 2026-07-17: addressable active-ban resource** ([ADR-0009](adr/0009-api-v2-bid-moderation.md)) — list is bounded newest-first; POST accepts only `bidId`, derives the canonical bidder and returns the created ban; DELETE removes `/{bidId}` with 204. Migration 00025 deduplicates by retaining the newest row and enforces one active ban per bid, making 409 duplicate creates and item identity race-safe. `AdminKey` preserves `X-Admin-Key` / `ADMIN_API_KEY` with fail-closed 503/401 and independent 2/5 write buckets. |
...
| D24 | Integrating post-fork upstream production changes | preserve legacy paths / accept rewrite and drop behavior / port behavior into modern owners with compatibility + canonical v2 | **decided 2026-07-23: modern-layout semantic port** — rebase keeps `rwcg/` and `previous-code/` deleted, then ports each upstream behavior into its modern owner behind the existing quality gates. Production-only v1 additions (AxiomZero marketplace) keep exact compatibility routes and receive canonical typed v2 resources; V3 events coexist by topic with V2; evented configuration drift is observed rather than fabricated; event-less snapshots use an audit table; monitoring keeps modern atomic/context-safe implementations while adopting upstream freshness/supervisor semantics. Historical synthetic rows are never deleted automatically because rollout ordering is operational, not schema-only. |
...
|---|---|---|
| 2026-07-23 | `ea22b797` + `a29236f4` + `86587400` | **Upstream-production-convergence sprint (§8.10; D24):** rebased the 80-commit modernization stack onto latest upstream `a1eb87d6` while retaining the exact pre-rebase modern tree, then ported every upstream-only behavior into modern owners rather than resurrecting `rwcg/`. **V3 (`ea22b797`):** authoritative ABI artifact + regenerated binding; reversible migration 00026; dual V2/V3 claim topics; multi-NFT claims; normalized 90/10 bid rewards; admin types 40–44; bounded claim-time/startup champion-duration persistence; live-state audit; synthetic-write contract sync replaced by block-pinned drift reporting; V3 mechanics/configuration and exact fields across v1/v2/FAQ. **Marketplace (`a29236f4`):** one collection-scoped pgx repository now powers unchanged RandomWalk behavior, three exact AxiomZero v1 compatibility routes and four canonical Cosmic Signature v2 resources with exact wei and endpoint/collection/sort-bound cursors; migration 00027 adds concurrent collection indexes; catalog counts 142→145 v1 and 97→101 v2 GETs. **Monitoring (`86587400`):** six event DB slots; atomic `#TS=` anomaly heartbeat; bounded backward-compatible freshness parser/UI/alarm; guarded transition loop; persistent hardened systemd one-shot timer. **Proof:** every commit passed the managed race coverage hook; final generation, build, vet, lint 0, `go fix`, race+shuffle unit/integration, migrations, 74-target fuzz smoke, systemd/shell tests and govulncheck pass. Metrics: LOC **196,058→202,011**, test files **402→411**, fuzz **73→74**, legacy internal **92.34%** (31,585/34,205), handwritten internal **95.55%** (25,167/26,340), all production **95.14%** (28,817/30,288), combined changed code **95.89%** (1,702/1,775). Remote feature branch intentionally remains at `2b61953e`; no push. |
| 2026-07-23 | — | **V2-operational-readiness sprint (§8.9 added and completed; D23 decided):** `opsctl smoketest` is now a typed `v2`/`v1`/`both`/`operational` engine with v2 as the D6-safe default. The full canonical suite reads the embedded OpenAPI document, discovers all **97 GET operations** (the five writes are excluded by method), binds production values for every path/required query including RandomWalk token IDs, enum pools and ISO date windows, requests `limit=1` pages, paces at the public 50 rps budget, requires 200 and validates each response body/content type against its operation schema; any new unmapped required parameter, RFC 9457 problem, schema drift, timeout/dependency error or `Deprecation`/`Sunset` header fails with the operation ID. The frozen envelope-aware **142-request v1** test remains explicit; `both` preserves per-suite + aggregate failures. The no-database six-request operational suite checks health/readiness/version plus stable CosmicGame rounds/counters and RandomWalk statistics, deliberately excluding the five contract-cache resources whose correct degraded behavior is 503; `/readyz` remains DB+draining only. **D6 enforcement:** the tiny deprecated-prefix classifier moved to `internal/api/policy`, shared by router headers, Prometheus labels, smoketest and srvmonitor without an API-router dependency; srvmonitor now aggregates malformed/deprecated internal/public URL errors at startup, while its probe semantics stay unchanged. `.env.example` and runbooks publish `/readyz` + stable-v2 examples. Compose gained an apiserver healthcheck and opt-in one-shot smoke profile; topology validation passes, while runtime startup here was externally blocked by registry authorization on the pre-existing goose image. **Tests:** exact catalog⇄OpenAPI equality/stable-order/no-write/no-v1 assertions, synthetic spec-drift fail-closed branches, request/response/schema/problem/body/header matrices, pacing/cancellation and command setup/cleanup tests, srvmonitor URL matrix, plus a race-enabled integration proof that drives all 97 probes through the seeded production router, real PostgreSQL and fixture-coherent chain with **97/97 valid 200s**. Fuzz targets 71→73 cover URL binding/escaping and deprecation determinism (dedicated 5s runs green). **Verification:** build/unit, full race+shuffle unit and integration suites, lint 0, vet, both `go fix -diff` sets, generation drift and working-tree coverage gate pass; Compose execution alone is blocked as noted. Metrics: LOC **194,672 → 196,058**, test files **399 → 402**, fuzz targets **71 → 73**, legacy internal **92.39%** (30,528/33,042), handwritten internal **95.47%** (24,347/25,502), all production **95.07%** (27,971/29,422), changed executable Go **97.76%** (305/312); floors unchanged because no metric cleared its next 0.1 step. |
| 2026-07-23 | — | **Bounded-time sprint (§8.8 added and completed; D22 decided):** after §8.6 (no orphan work) and §8.7 (no unbounded input), the third resource closes — no API request, database statement or outbound call can wait forever. **API:** `common.RequestDeadline` in the shared chain puts one fixed 30s deadline on every request context (`routes.RequestDeadlineFor`; the FAQ proxy is the one exempt family — bounded by its own 180s client + 4 MiB cap); a post-deadline internal error is intercepted at the header boundary and rendered as the family 503 — RFC 9457 `request-timeout` problem under `/api/v2/` (from `common.ProblemTypeBase`, byte-pinned) or the legacy envelope — with **zero churn across the ~300 v1/v2 rendering sites**, while late successes and client cancels pass through untouched; the middleware's request clone copies the mux `Pattern` back so outer metrics/access-log labels keep working (pinned), and `rwcg_http_request_timeouts_total{method,route}` counts interceptions. The public listeners keep `WriteTimeout=0` under a now-truthful rationale (buffered large v1 arrays to slow clients; execution is bounded by the deadline). **DB (defense in depth):** `store.Config` gains `StatementTimeout`/`IdleInTxSessionTimeout` GUCs — apiserver 30s/5m, cg-etl/rw-etl/notibot/imggen-monitor 60s/5m, operator CLIs deliberately unset; integration proves `SHOW` round-trips, a runaway `pg_sleep` dying with 57014 and a healthy pool after, prompt client-side cancellation, and termination+recovery of a transaction stalled between statements. **ETL:** `indexer.Config.RPCCallTimeout` (60s) wraps the `Client` seam in a `boundedClient` decorator covering every call incl. the in-transaction lookups on the shutdown-immune `WithoutCancel` context — a black-holed endpoint now yields ordinary batch failures (backoff → batch shrink → breaker → restart) where it used to hang the ETL **forever, un-interruptible even by SIGTERM, without recording a single failure**; synctest proves per-method bounds, earlier-deadline composition, WithoutCancel bounding and the hung-fetch breaker trip, and an integration test proves a hung mid-transaction RPC rolls the block back with zero residue and converges on retry. New [internal/ethcall](../internal/ethcall) bounds `bind.ContractCaller`s (15s): the CG handlers wrap at construction (pinned), the startup contract sync runs over a read-bounded backend with ctx-carrying `CallOpts` (previously `context.Background()`), and v1's one request-time RPC (live special winners) joins the contractstate `rpcReadTimeout`. **Outbound sweep:** Twitter's OAuth fallback client bounded at 2m (never `http.DefaultClient` again), WhatsApp 30s, Discord sink 30s/call (rate-limit waits stay outside; hang test), autobid's default dial injects a 30s-per-exchange HTTP client under its whole RPC surface, srvmonitor's RPC probe joins the other monitors at 10s, `logscan` bounds each fetch at 60s (`FetchTimeout`; timed-out fetch halves and retries) and opsctl's chain tools dial through 90s-per-exchange clients (layering pinned above logscan's bound). `BenchmarkRequestDeadline` baselines the wrapper: 1,006ns bounded vs 764ns exempt full-router exchange (+~240ns, +6 allocs); MaxRequestBody/RateLimiter neighbours re-ran at or below their baselines. **Verification:** build, vet (both tag sets), lint (0), both `go fix` gates, full race+shuffle unit and integration suites (zero failures, all goldens byte-identical), race-enabled coverage gate PASS, govulncheck 0 reachable; the 71-target 1s fuzz smoke hit the known worker stop-deadline class on two cursor/mapping targets with no crashers — both passed dedicated 10s runs (689K/769K execs). Metrics: LOC **192,367 → 194,672**, test files **384 → 399**, benchmarks **10 → 11** funcs (44 → 46 leaf), unit internal **66.5%**, legacy internal **92.35%** (30,312/32,823; floor 92.3%), handwritten internal **95.44%** (24,131/25,283; floor 95.4%), all production **95.04%** (27,747/29,195; floor 95.0%), changed executable Go **98.23%** (222/226; only the two unreachable timeout-marshal fallbacks uncovered). Floors unchanged — no measurement cleared its next 0.1 step. |
| 2026-07-22 | — | **Edge-hardening sprint (§8.7 added and completed; D21 decided):** the process edge accepts no unbounded client input. `common.MaxRequestBody` (fixed 1 MiB — D21, no env knobs per D9) joined the shared chain: declared oversized bodies answer 413 without a byte read, undeclared ones fail at first read via `http.MaxBytesReader`, and each family renders its own shape — the generated v2 decoders map `*http.MaxBytesError` to an RFC 9457 `request-too-large` problem pinned **byte-identical** to the middleware's pre-check rendering (`common.ProblemTypeBase` is now the single source v2 problem types build from), the FAQ proxy and everything else answer the legacy envelope, and frozen v1 binds keep their 400 error-string shape. The three body-accepting v2 operations declare 413 (`RequestTooLarge`, spec v0.19.0 → v0.20.0, regenerated + drift-gated); a spec test pins 413 ⇄ requestBody equivalence both directions, and an integration matrix walks every non-GET route on the production router (11 today, discovered dynamically) plus per-family undeclared-overflow and cap-minus-one-must-bind cases. **FAQ proxy hygiene:** buffered upstream responses cap at 4 MiB (over → 502 without relaying, exactly-at-cap relays byte-identically) and the request-build 502 no longer echoes `err.Error()`. **Transport:** public TLS listeners set `MinVersion` 1.2 (boot test refuses a 1.1-only handshake, accepts 1.2, on both listeners) and every `http.Server` — public, internal, ETL metrics — routes `ErrorLog` through the process slog logger at WARN. **Pool observability:** `store.PoolCollector` exposes all thirteen `pgxpool.Stat` signals as `rwcg_db_pool_*`; the apiserver (default registry) and both ETLs (injected registries) register it with paired shutdown unregistration (D20 ownership, gathered-after-return tests) and loud registration-conflict startup errors covered in all three binaries. Exposition is pinned metric-by-metric (`CollectAndLint`/`CollectAndCompare`), a real-workload store integration test proves counters advance and gauges reconcile, the apiserver boot test scrapes the series through the real internal listener, and [operations.md](operations.md) documents the body-cap policy plus saturation PromQL. `BenchmarkMaxRequestBody` baselines the per-request wrapper (~sub-µs full-router exchange; rate-limiter/Compress/ETag re-ran clean) in [benchmarks.md](benchmarks.md). **Verification:** build, vet (both tag sets), lint (0), both `go fix` gates, deterministic regeneration, full race+shuffle unit and integration suites, race-enabled coverage gate and fuzz smoke pass; govulncheck surfaced one newly-published reachable advisory (GO-2026-5970, x/text infinite loop reachable through pgx's SASL path) — `golang.org/x/text` bumped v0.38.0 → v0.39.0 (x/mod, x/tools follow), scan back to 0 affected. Metrics: LOC **191,090 → 192,367**, test files **379 → 384**, benchmarks **9 → 10** funcs (41 → 44 leaf), unit internal **66.4%**, legacy internal **92.31%** (30,196/32,710; floor 92.3%), handwritten internal **95.41%** (24,015/25,170; floor 95.4%), all production **95.01%** (27,609/29,060; floor 95.0%), changed executable Go **98.36%** (120/122; only the unreachable problem-marshal fallback uncovered). Floors unchanged — no measurement cleared its next 0.1 step. |
...
| 2026-07-17 | — | **API-v2 bid-moderation sprint (§6.2.1 genuinely complete; D14 decided, [ADR-0009](adr/0009-api-v2-bid-moderation.md)):** a completion audit found the backend-owned v1 `get_banned_bids`, `ban_bid` and `unban_bid` behaviors absent from the published “complete” mapping. [openapi-v2.yaml](openapi-v2.yaml) v0.18.0 → v0.19.0 and the strict router 99 → **102** operations: public cursor-paginated `GET /api/v2/cosmicgame/moderation/banned-bids`, `AdminKey`-secured POST `{bidId}` → typed 201, and DELETE `/{bidId}` → 204. The server derives the bidder from the canonical indexed bid, checksums addresses, converts Unix timestamps to RFC 3339 and rejects corrupt repository identity/order/cardinality opaquely. Migration 00025 deterministically keeps the newest duplicate and replaces the old lookup index with a unique `bid_id` invariant; store methods provide stable descending-ID pages, insert-returning, canonical bidder lookup and found-aware removal while frozen v1 methods keep their wire behavior (duplicate v1 creates map the new conflict to their historical idempotent 201). The scopes-driven middleware now handles both `AdminKey` and `RankingAdminKey`, compares fixed-width SHA-256 values in constant time, fails closed with 503/401 and isolates create/delete/ranking token buckets. **Tests:** 350 → **353** test files, 69 → **70** fuzz targets, 529 → **546** v2 goldens; migration down/up deduplication + uniqueness, store page walks and insert-during-traversal stability, full handler/mapping/problem/auth/rate-limit matrices, generated middleware-response visitors, full-router create/conflict/delete lifecycle with fixture restoration, keyless-router 503, production boot wiring and hostile cursor fuzzing. All pre-existing v1/v2 goldens remain byte-identical. Full build, vet, `-race -shuffle` unit and `-race` integration suites, deterministic generation, lint (0 issues under plain and integration tags), govulncheck and the **70-target 10-second fuzz fleet** pass; coverage gate passes. **Metrics:** LOC 183,565 → **185,354**; legacy internal **92.19%** (floor 92.1), handwritten internal **95.23%** (floor 95.1), all production **94.56%** (floor 94.5), changed executable Go **98.94%** (187/189; only the validated-cursor encode defense arm is unreachable). |
| 2026-07-18 | — | **V2 later-store-failure hardening sprint (§6.2 test suite):** the full-router integration harness now covers the failure stage that cancelled-context tests cannot reach: an earlier resolver/existence query succeeds against real pgx/PostgreSQL, then a later domain relation is hidden. One 29-case sequential matrix covers six current-round/round/claims shapes, twelve CosmicGame user history/staking/activity shapes, three token-directory/global-staking shapes, five RandomWalk token/user shapes and three ranking shapes; four later stages that cannot be isolated safely by pre-request DDL remain pinned by their existing unit fakes. Every injected response is asserted as an exact five-field RFC 9457 500 (`application/problem+json`, canonical internal type/title/status/detail, query-free path instance), scanned for relation/SQLSTATE/driver/database/credential leakage and response-validated against the canonical OpenAPI operation. The shared v1/v2 fault seam now quotes identifiers, bounds rename/restore DDL to five seconds, guarantees cleanup and resets the pgx pool on both transitions so cached statement state cannot mask the fault. No production behavior, OpenAPI operation or golden changed. Targeted `-race` matrix, full build, `-race -shuffle` unit suite, `-race` integration suite, vet, deterministic generation and lint (0 issues) pass; the CI-equivalent race coverage gate passes at legacy internal **92.18%** (29,735/32,256), handwritten internal **95.22%** (23,708/24,899) and all production **94.55%** (27,159/28,726). Test files 353 → **354**; LOC 185,354 → **185,732**. |
| 2026-07-18 | — | **Transactional-ingestion sprint (§7 complete — the last in-repo engineering checkbox closes; D15 decided, [ADR-0010](adr/0010-transactional-ingestion.md)):** every block's ETL writes and its processing watermark now commit in **one PostgreSQL transaction**, so readers can never observe a partially applied block, the watermark can never disagree with the data it acknowledges, and delete-then-insert replay idempotence is demoted from load-bearing to defense in depth. **The querier seam the 2026-07-09 deferral asked for:** `store.Querier` (Exec/Query/QueryRow; satisfied by `*pgxpool.Pool` and `pgx.Tx`) plus context-scoped `Store.InTx(ctx, fn)` — fn's context carries the transaction, `Store.Querier(ctx)` resolves pool-vs-transaction per call **with an owner check** (a foreign Store's methods never adopt another Store's transaction), nested calls join, commit/rollback stay with the outermost call. All 474 repo methods (both domains) and 23 base Store methods moved onto per-call resolution with **zero signature changes** — the mechanical `pool()` → `q(ctx)` rewrite left every query, wire shape and API path untouched (API request contexts never carry a transaction). **Tx-aware address cache:** ids created inside a transaction live in a per-transaction overlay — rollback discards them (a poisoned shared LRU would feed dangling FK ids to every retry), commit flushes them; `LookupOrCreateAddress` now inserts with `ON CONFLICT (addr) DO NOTHING` + re-read because a lost insert race inside a transaction previously aborted it (SQLSTATE 25P02) — the regression test synchronizes the losing session against `pg_stat_activity` so the conflict branch is hit deterministically, then proves the transaction stays healthy. **Engine:** `processBatch` groups fetched logs per block; `processBlock` runs blockops → handler dispatch → `Progress.SetLastBlock` inside `InTx`, so committed blocks acknowledge themselves (the separate partial-progress ack write is deleted); chain-split rollback+reinsert joins the block's transaction; event-free tail ranges keep their plain watermark write; `events_total`/watermark metrics emit only after commit and Begin/Commit failures surface as the new `commit` stage of `rwcg_etl_batch_failures_total` (operations.md updated). Both fixture harnesses ingest through `InTx`, proving all 85 handlers behave identically inside the production transaction scope. **Deliberate behavior change:** the watermark now advances once per block-with-events instead of once per batch (the shutdown-mid-batch test pins `[100 101 102]` where it pinned `[102]`). **Tests:** 354 → **357** test files — the store `InTx` integration suite (commit visibility/cache flush via delete-behind-the-cache, rollback write+cache discard, wrong-Store isolation, nested joins, panic rollback, begin/commit failure arms, repo-join proof, the 25P02 conflict regression, read-only status-seed failure arms), the indexer **atomicity matrix** (block/transaction/event_log/process/watermark stage injections each proving zero-residue rollback — no block/transaction/evt_log/address rows, no watermark movement — plus retry convergence, committed-block isolation during multi-block batches and the `commit` stage), and overlay-semantics unit tests (`-race`). All **196 v1 parity + 12 error + 546 v2 goldens and every store/ETL fixture golden byte-identical (zero regenerated)**; full-router, boot and reorg suites green. **Benchmarks:** new `BenchmarkIngestBlock` pair pins the wrapper at ~330µs (~9%) per three-log block against the identical autocommit pipeline (in production one fsync per block replaces one per statement); CG statistics suite at-or-faster than baselines, and a serial worktree A/B put the ranking queries dead even (178 vs 177µs) with byte-identical allocations — the per-query querier resolution is one context lookup. Full build, vet (both tag sets), `-race -shuffle` unit suite, `-race` integration suite via the coverage gate, lint (**0 issues**), `generate-check` and targeted 10s fuzz runs (`FuzzConnStringEscape`, `FuzzLoadValue`) pass. **Metrics (authoritative race profile):** LOC 185,732 → **186,756**, legacy internal **92.18% → 92.25%** (floor 92.1 → 92.2), handwritten internal **95.22% → 95.29%** (floor 95.1 → 95.2), all production **94.55% → 94.61%** (floor 94.5), changed executable Go **99.75%** (794/796; the two uncovered statements are the status-seed re-read defense arms). |
| 2026-07-18 | — | **One-driver sprint (D16 decided: the ops toolchain drops `database/sql` + `lib/pq`; pgx becomes the repository's only PostgreSQL driver, depguard-enforced):** the last non-pgx database code — `internal/ops/{dbverify,archive,txcollector,cstscan,smoketest,assets}`, `internal/toolutil` and ten `opsctl` command files — moved onto pgx-native queries behind narrow per-package `Querier` interfaces (the `freezer/verify` precedent; `*pgxpool.Pool` satisfies every one, and `pgx.Tx` still fits). **SQL byte-identical except three deliberate casts:** archive export's NUMERIC copies (`transaction.value`/`gas_price`, `block.cash_flow`) gained `::text`, reproducing exactly the text rendering the old text-protocol driver received (pgx's binary protocol has no numeric→string scan path by design); `pq.Array` became native slice binding everywhere; the shared-fixture export/verify integration tests pin both equivalences against real PostgreSQL. **Node-fill:** the nine `*sql.Stmt`s dissolved into pgx's per-connection statement cache — `PrepareWriter` no longer touches the database (its prepare-failure arms died with it), exists-checks map `pgx.ErrNoRows`, and the `stdlib.OpenDBFromPool` bridge in `cmd_archive_node_fill.go` is deleted: **one pgxpool now serves both the `store.Store` address cache and the archive statements**, closing the §5.3-era "tool-local DSN handle" note. **Command edge:** the ten deps seams swapped `sql.Open` for a shared `openOpsDB(maxConns)` opener over `pgxpool.ParseConfig` (passwords redacted in parse errors — lib/pq could echo the DSN); export/verify keep their historical 2-connection bound, tx-collector verify its 4, and the previously **unbounded** handles (db verify/evtlog-diff, smoketest, cst-auction-len, assets) are now explicitly bounded. Dead `toolutil.OpenPostgresFromEnv` deleted (its conn-string builder and `DATABASE_URL` precedence live on, tests intact). **Tests (the sprint's bulk):** the three scripted `database/sql` driver harnesses (archive's 248-line driver, dbverify's 944-line suite, the cstscan/txcollector/smoketest/assets connector fakes and opsctl's `testSQLConnector`) were replaced by one shared, self-tested scripted pgx querier, [internal/testutil](../internal/testutil) `ScriptedPgx` — ordered ops with SQL-substring/arg-count assertions, per-row iteration faults, QueryRow no-rows → `pgx.ErrNoRows`, exec tags, OnCall hooks and a pool-shaped `Ping`/`Close`/`Closed` surface the command wiring tests assert against; every pre-existing error-branch case (query/scan/rows.Err/mid-iteration/cancellation) was preserved through the port, and the fake's own conversion matrix, row semantics and violation reporting are unit-tested. The dbverify two-schema comparison, archive export/verify/node-fill repair, txcollector, smoketest params and assets seeds integration suites now run on `testdb.Pool` handles; the freezer prod-DB verify harness (`decode/verify_db_test.go`) and its `pq.Array` usage moved to a pgx pool as well. New unit tests cover the pool opener (MaxConns bound without dialing, malformed-DSN secret redaction) and the node-fill writer's exec/exists/cleanup-sequence branches. **go.mod:** `github.com/lib/pq` (maintenance-mode, formerly direct) is gone — `go list -deps` over every binary shows zero lib/pq packages — and `.golangci.yml` gained a `depguard` deny rule so it can never silently return (the gin-removal ratchet class). `database/sql` survives only as driver-free stdlib value types (`sql.NullString`/`NullInt64`, which pgx scans natively) and the goose/testfixtures stdlib bridge in `internal/testdb`. **Verification:** build, vet (plain + integration tags), full `-race -shuffle` unit suite, the full `-race` integration suite via the coverage gate, `make generate-check`, lint **0 issues** (with `depguard` active), govulncheck (0 reachable), and a 10s `FuzzConnStringEscape` run all pass; no store/API/ETL code changed, so all parity/v2/store/ETL goldens are untouched by construction (the gate's full-router suites ran green). **Metrics (authoritative race profile):** LOC 186,756 → **186,961** (the shared fake + its suite outweigh the deleted plumbing), test files 357 → **359**, legacy internal **92.25% → 92.26%** (floor 92.2), handwritten internal **95.28%** (floor 95.2), all production **94.61%** (floor 94.5), changed executable Go **99.69%** (321/322; the one uncovered statement is the pool-construction failure arm behind an already-validated config). |
| 2026-07-18 | — | **Modern-idioms sprint (§8.4 extended: Go 1.26 `go fix` sweep + permanent CI gate; lint frontier v2 at ten more linters; D17 decided — benchmarks on `b.Loop`):** the stdlib modernize fixer suite ran to a fixed point under both build-tag sets (56 files): `interface{}`→`any`, `min`/`max` clamps, `maps.Copy`, `slices.Contains`, `strings.SplitSeq`/`CutSuffix`, `reflect.TypeFor`, `sync.WaitGroup.Go`, Go 1.26 `new(expr)` (the four hand-rolled pointer helpers it orphaned are deleted) and no-op struct `omitempty` removal in the WhatsApp wire types (encoding/json ignores `omitempty` on structs; the conformance suite pins the payloads). **Legacy `sort` retired:** ~40 call sites became `slices.SortFunc`/`SortStableFunc`/`Sort` with `cmp.Compare`/`cmp.Or` comparators; the OAuth1 signer's `byKeyValue` `sort.Interface` boilerplate is one RFC 5849-documented comparator pinned by the known-answer vector; `sort` and `math/rand` v1 are absent from the non-generated tree (srvmonitor's image spot-check seam rides `math/rand/v2.Int64N`). **Exit-path hardening (gocritic `exitAfterDefer`):** all ten signal-handling binaries (`apiserver`, both ETLs, `notibot`, `imggen-monitor`, `rwalk-alarm`, `freezer-scan`, `freezer-verify`, `opsctl`, `srvmonitor`) restructured so `main` is a thin caller of `runMain() error` behind a stubbable `osExit` seam — every deferred cleanup (signal release, log rotation, terminal restore) now runs before the exit code is decided. srvmonitor's panic recovery converts TUI panics into ordinary errors, and its display/poller gained injectable seams: setup failure, display-init failure, the recovered panic, the full dashboard run (scripted 'q' exit) and every binary's `main` failure arm are all under test (10 new `runmain_test.go` files). **Lint frontier v2:** `gocritic`, `perfsprint`, `sloglint`, `recvcheck`, `embeddedstructfieldcheck`, `iface`, `mirror`, `exptostd`, `fatcontext` and `nilnesserr` enabled at zero findings (~120 fixed: constant `fmt.Errorf`→`errors.New`, Sprintf→`strconv`/concatenation, `slog.DiscardHandler`, covergate's mixed value/pointer receivers unified, two aliasing cross-slice `append` concatenations→`slices.Concat` in integration tests, one deliberate whitespace map key justified with `//nolint`). **CI ratchet:** the lint job now fails on any pending modernization via `go fix -diff` (plain and `-tags=integration`); `make fix` applies both locally (CONTRIBUTING updated). **Benchmarks (D17):** all 41 leaf benchmarks moved from `for range b.N` to `b.Loop()` and the whole table was re-measured in one sitting — allocation counts byte-identical everywhere, every latency at-or-below its baseline band ([benchmarks.md](benchmarks.md) re-based with the loop-idiom comparability caveat). **Verification:** build, vet (both tag sets), full `-race -shuffle` unit suite, the full `-race` integration suite via the coverage gate, `make generate-check`, lint **0 issues** with the enlarged set, govulncheck (0 reachable) and a 5s-per-target run of the whole 70-target fuzz fleet all pass; parity/v2/store/ETL goldens byte-identical throughout. **Metrics (authoritative race profile):** LOC 186,961 → **187,441**, test files 359 → **369**, legacy internal **92.27%** (floor 92.2), handwritten internal **95.28% → 95.31%** (floor raised 95.2 → 95.3), all production **94.61% → 94.92%** (floor raised 94.5 → 94.8 — the `main()` bodies that made up the residue are now tested), changed executable Go **96.21%** (508/528; the residue is deep error arms whose lines the sweep merely re-spelled). |
