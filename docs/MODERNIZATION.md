...
Measured 2026-07-16 (global-staking sprint; every CosmicGame v1 endpoint
group now has a bounded v2 replacement — §8.5 code documentation, the
§6.2.1 RandomWalk slice and §6.3 are the open sections). Update after each
phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **167,658** (+4,746 this sprint: exact keyset-paginated global staking store pages, injected handlers/mappers/cursors, five layered test files, 39 HTTP goldens and benchmark coverage; generated `internal/api/v2/api.gen.go` is separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **331** | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
| Fuzz targets | 0 | **63** (+4: endpoint-scoped global staking event and live-membership cursors, the exact deposit ledger cursor and the round-scoped reward cursor; cross-resource/pool/round payloads fail closed) | **25+** (see §4.4) — met | `rg "func Fuzz" internal cmd contracts -c` |
| Benchmarks | 0 | **6** (35 sub-benchmarks; baselines in `docs/benchmarks.md`) | keep green vs baselines | `rg "func Benchmark" cmd internal -c` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **92.03%** (floor 92.0%; the 39 real-router HTTP cases plus generated enum/binding conformance keep the raw generated-code-inclusive metric above its ratchet after nine operations) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **94.88%** (floor 94.7% → 94.8%; generated API + test-only harnesses excluded by ADR-0006) | **≥90% — met**; commit gate active, next target 95% | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **94.17%** (floor 94.0% → 94.1%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **97.75%** (global-staking sprint; the fifteen uncovered statements are five logically unreachable cursor-encode failure arms and three defensive row-scan error arms) | **≥95%** | `make coverage-check` |
| Queries on sqlc | 0 | 0 — scaffolding retired with Phase 1 (D7 amended: hand-written pgx everywhere) | n/a | n/a |
| Routes on stdlib router | 0 | **all** — frozen v1 (188 OpenAPI operations incl. `/version`) plus 75 generated v2 operations (round resources, exact statistics/claims, participant/bidding analytics, five contract/live resources, the complete user surface, global token/CosmicToken/marketing directories and nine global staking resources), health, host-dispatched metadata and env-gated static assets on `net/http` via `internal/api/httpx`; **gin is out of the build graph** | all (v1 compat + v2) — active | route-drift tests + `go list -deps ./cmd/... ./internal/... \| rg -c gin-gonic` |
...
- [~] Publish v2 spec + changelog mapping every v1 path to its v2 replacement
      *(dashboard, user profile/bid, user winnings/donations, user staking,
      user activity, global token/CosmicToken/marketing and global staking
      mappings published in [api-v2-migration.md](api-v2-migration.md) —
      ...
      retired with the request-time RPC (D11); only the RandomWalk group
      remains)*
