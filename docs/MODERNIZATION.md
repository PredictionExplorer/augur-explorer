...
Measured 2026-07-17 (bid-moderation-v2 sprint: the three omitted
`get_banned_bids`/`ban_bid`/`unban_bid` behaviors now form one bounded,
typed active-ban resource; migration 00025 enforces one ban per bid, the
shared strict middleware handles both admin-key schemes, and ADR-0009 records
DELETE semantics. The canonical v2 surface is 102 operations and its mapping
is now genuinely complete. Every remaining unchecked item is external
(frontend migration), production-gated (v1 removal, run-loop.sh deletion,
prod-RLP replay) or a decision (D1).) Update after each phase.

| Metric | Baseline (start of project) | Current | Target | How to measure |
|---|---|---|---|---|
| Hand-written Go LOC (`cmd/` + `internal/`) | ~60,000 (plus 124k frozen legacy) | **185,354** (+1,789 this sprint: moderation store/handler/middleware code plus exhaustive unit, migration, full-router and boot tests; generated `internal/api/v2/api.gen.go` and `contracts/*/bindings.gen.go` are separate) | n/a (informational) | `rg --files internal cmd -g '*.go' -g '!internal/api/v2/api.gen.go' \| tr '\n' '\0' \| xargs -0 wc -l \| tail -1` |
...
| Test files | 17 | **353** (+3: bid-moderation handler/cursor and full-router suites; incl. 4 `Example*` files and the contracts artifact drift suite) | 100+ — met | `rg --files -g '*_test.go' \| wc -l` |
...
| Legacy `internal/` integration coverage (enforced) | n/a | **92.19%** (29,736/32,256; floor 92.1%) | ratchet only; never lower | `make coverage-check` |
| Handwritten production `internal/` coverage (enforced) | n/a | **95.23%** (23,712/24,899; floor 95.1%; generated API + test-only harnesses excluded by ADR-0006) | **≥95% — met**; commit gate active | `make coverage-check` |
| Handwritten all-production coverage (`cmd/` + `internal/`, enforced) | n/a | **94.56%** (27,163/28,726; floor 94.5%; the remaining uncovered code is `main()` bodies and shutdown-error plumbing) | **≥90% — met**; ratchet only | `make coverage-check` |
| Changed executable Go coverage (enforced) | n/a | **98.94%** (187/189; the two uncovered statements are the logically unreachable validated-cursor encode failure arm) | **≥95%** | `make coverage-check` |
