...
```go
aid, err := Lookup_or_create_address(addr, blockNum, txId)  // Returns BIGINT ID
```
...
The schema is managed by goose migrations in `db/migrations/`:

```bash
GOOSE_DBSTRING="postgres://user:pass@host:5432/db" make migrate-up
```

See [operations.md](operations.md) for adopting migrations on an existing
production database.
